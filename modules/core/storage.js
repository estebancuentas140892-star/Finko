...
function _migrate(raw) {
  ...
  // v21 → v22: nueva colección de ingresos puntuales (NAV.A1, ADR 024). El
  // usuario existente arranca sin ingresos puntuales; el resto no cambia.
  // Idempotente: si ya es un array, no se toca.
  if ((typeof data._version === 'number' ? data._version : 1) < 22) {
    if (!Array.isArray(data.ingresosPuntuales)) {
      data.ingresosPuntuales = [];
    }
  }

  // v22 → v23: accesos rápidos personalizables de Inicio (IN.4a, ADR 028 D2).
  // El usuario existente arranca con el default (Mis cuentas, Ahorros,
  // Límites de gasto); puede personalizarlo después. Idempotente: si ya es
  // un array, no se toca.
  if ((typeof data._version === 'number' ? data._version : 1) < 23) {
