...
> Estado al 2026-07-03: **épica MC.7 completa (MC.7a a MC.7f).** MC.7d se entregó en dos partes: primero la checklist accionable de Necesidades (R1/R4/R5), luego el shell paginado (Necesidades → Ahorro/Deudas/Inversiones → Estilo de vida, confirmación única al final) y el presupuesto de ahorro sobre el remanente real (R3, helper `presupuestosSobreRemanente` con tope en la sugerencia teórica del split). MC.7e sumó, dentro del paso final "Estilo de vida" y con 2+ cuentas activas, un reparto opcional hacia otras cuentas (transferencias internas, sin marcar nada por defecto). MC.7f cerró el pulido: copy consistente por paso, foco movido al contenedor del paso al avanzar/retroceder (WAI-ARIA APG para asistentes multi-paso), el indicador "Paso X de N" se omite cuando solo hay un paso, la sugerencia de ahorro ya no aparece si no hay ninguna fila de Ahorro donde ponerla, y una transición sutil de opacidad al cambiar de paso (respeta `prefers-reduced-motion`). Solo quedan pendientes opcionales sin relación directa con el asistente (MC.6c, MC.7g).
>
