...
> Estado al 2026-07-03: **MC.7a a MC.7e entregados**. MC.7d se entregó en dos partes: primero la checklist accionable de Necesidades (R1/R4/R5), luego el shell paginado (Necesidades → Ahorro/Deudas/Inversiones → Estilo de vida, confirmación única al final) y el presupuesto de ahorro sobre el remanente real (R3, helper `presupuestosSobreRemanente` con tope en la sugerencia teórica del split). MC.7e llegó el mismo día: dentro del paso final "Estilo de vida", con 2+ cuentas activas aparece un reparto opcional hacia otras cuentas (transferencias internas, sin marcar nada por defecto: todo el remanente sigue en la cuenta de origen salvo que el usuario mueva algo explícitamente). Con una sola cuenta el paso sigue siendo puramente informativo. Solo queda el pulido opcional MC.7f.
>
