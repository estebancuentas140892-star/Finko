...
- Abierto por 16: **el monitor de renta (K.3) queda sin auditar**. Es una funcion fiscal completa (topes en UVT, criterios, veredicto y nudges) y la ficha se nego a examinarla de pasada: la **18** decide si entra al alcance de la auditoria o se documenta aparte. Ademas, **R88 encontro su segundo caso** y llega lista para confirmar con redaccion ampliada (lo que encabeza una vista describe su conjunto **y su periodo**), y **R90 queda acotada a Movimientos y al panel de prioridades**: la 18 verifica que ninguna vista sin auditar mezcle direcciones. **Cerrado sin tarjeta:** la pregunta de la ficha 12 sobre mudar el seguimiento de los tres grupos a Analisis (no, la seccion esta clavada a `hoy()` y no tiene selector de mes) y la de la 14 sobre como Analisis presenta "Por cobrar" (el aviso condicional ya existia desde ANL.3, asi que la redaccion de "Solo seguimiento" se confirma tal cual).
- Abierto por 15: **historial fechado para metas y reservas**, la otra mitad de M1 y lo unico que el ledger no pudo completar: hoy solo guardan `montoActual`, sin fecha por aporte, asi que un aporte a una meta no sale en ninguna vista cronologica. Es **modelo de datos** (schema, migracion y probablemente las fichas 09 y 10, ya cerradas) y el precedente esta escrito: los prestamos ganaron `abonos[]` en el v34 justo por esto. Va a la **ficha 18**, junto con la redaccion final de **R90** (tres estados y alcance acotado a vistas con varias direcciones, ya con su primer caso independiente) y con que **"Consultar" aloja dos criterios distintos de solo lectura**: Calendario cedio sus acciones y Movimientos conserva editar y borrar, que en un ledger si son propias. **R88 llega a la ficha 16 con una sola aparicion: es su ultima oportunidad.**
- Abierto por 14: **candidata R90**, la direccion del dinero se señala con forma y no con color. La ficha la anoto **con cautela declarada**: sus dos apariciones son el mismo defecto visto dos veces, asi que las fichas **15** (Movimientos) y **16** (Analisis), las dos vistas que mezclan direcciones, la confirman o la tumban, y de ellas depende su alcance (regla general o arreglo local del panel de prioridades). A la **ficha 18** le llegan tambien el **cuarto pendiente del mismo tipo** que las fichas 05, 10 y 11 (los dos umbrales de aparicion del resumen y del historial por persona: revisar si "aparece cuando hay algo que decir" es pauta o coincidencia) y que **"Tu dinero" agrupa por tema y no por uso**, sin reabrir la ficha 03. La redaccion final del aviso "solo seguimiento" depende de como Analisis presente "Por cobrar", y se verifica en la **ficha 16**.
