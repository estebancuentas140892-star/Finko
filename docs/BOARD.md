...
- **Rebanadas (ADR 041):** **MC.13a cerrada el 2026-07-14** (`infra/vencimientos.js` mitad A: `ocurrenciasEnMes`/`ocurrenciasEnRango`/`ventanaDelCobro` + Agenda lo consume), **MC.13b cerrada el 2026-07-14** (mitad B: `FRECUENCIAS_APORTE`/`frecuenciaPrincipalIngresos`/`normalizarFrecuenciaAporte`/`diasPorPeriodo`/`etiquetaPeriodo`/`aportePorPeriodo` + Metas y Apartados borran sus copias; cero tests existentes tocados) y **MC.13c-1 cerrada el 2026-07-14** (`obligacionesYAportesDelCobro` puro: compone `ventana`/`vencidas`/`enVentana`/`aportes`; 40 tests, sin consumidores todavía), **el motor está completo**, ver CHANGELOG y [`contexto/mis-cuentas.md`](contexto/mis-cuentas.md) → **MC.13c-2** (abajo: la checklist lo consume, requiere 2 decisiones de Esteban) → **MC.13d** schema `cuentaId` en Ingreso (bump v26→v27) + form (independiente) → **MC.13e+** asistente v2 UI (puntos 9-20; depende de la decisión (a) para el flujo esporádico).
...
- Depende de : nada para MC.13c y MC.13d (el motor ya está construido); coordinar con PA.1 (conflicto (b)); decisión del conflicto (a) sobre NAV.A2b antes de MC.13e
- Modelo     : análisis/re-corte hecho con Opus 4.8 (ADR 041); MC.13a y MC.13b hechas con Opus 4.8 - Alto; MC.13c Opus 4.8 - Alto (composición de las dos mitades sobre datos reales), MC.13d Opus 4.8 - Extra (bump con migración), MC.13e+ Sonnet 5 - Alto por rebanada de UI
