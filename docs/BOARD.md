...
- Prioridad  : alta
- Estado     : **el motor está completo y en producción** (MC.13a-d + MC.13c-2, cerradas el 2026-07-14; [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md) aceptado parcialmente). **MC.13e-1 cerrada el 2026-07-15** (decisión (a)). **MC.13e-2 (análisis y re-corte) hecho el 2026-07-15**: el rediseño del asistente en sí (puntos 9-21 del brief) queda partido en las 7 rebanadas MC.13e-2a a MC.13e-2g de abajo, cada una verificable de forma independiente (regla 2.1).
...
#### MC.13e-2a - Copy, accesos cruzados y navegación del asistente
- Prioridad  : media
- Estado     : pendiente. Sin riesgo financiero, el primer pick natural (regla 2.1: empezar por el subset más chico).
- Objetivo   : agrupa 4 puntos de bajo riesgo y alta independencia del brief. (11) quitar el array `ctas` (accesos "Ver progreso del fondo", "Explorar/Aportar a inversiones", "Ver estrategia de deudas", "Ver tu seguimiento en Límites de gasto") del asistente; decidir si también sale de la tarjeta compacta (hoy es su único consumidor) o si ahí sí aporta contexto antes de abrir el asistente. (12) el aviso "recibiste tu ingreso" (subtítulo dinámico de `_renderTarjetaDistribuir`) y el resto de la tarjeta pasan a ser dos bloques visualmente independientes. (13) revisar el texto del `<legend>` "Reparte hacia tus necesidades, ahorros, deudas e inversiones..." (`_renderPanelDistribuir`), sobre todo si MC.13e-2b quita deudas del panel. (17) modernizar los botones Atrás/Siguiente/Distribuir (`.distribuir__nav`) con la identidad visual vigente (ADR 031/033: iconos, tokens, sin diseño nuevo).
- Secciones  : Mis cuentas
- Archivos   : `modules/dominio/tesoreria/logic/distribucion.js` (`sugerirDistribucionIngreso`, array `ctas`), `views/distribucion.js` (`_renderTarjetaDistribuir`, `_renderPanelDistribuir`), `styles/components/*.css` (`.distribuir__nav`, `.distribuir-card`)
- Depende de : nada
- Modelo     : Sonnet 5 - Medio (copy + CSS, cero lógica financiera)
...
- Depende de : conviene después de MC.13e-2b (menos ruido en el paso final); **bloqueada por la decisión de UX de Esteban**
- Modelo     : Opus 4.8 - Alto (lógica financiera + decisión de producto no trivial)
...
- Depende de : conviene última (reestructura el contenedor donde viven las demás rebanadas); depende de la decisión de handoff de diseño
