...
#### DV.1 - ADR de Dirección Visual premium (superficie, riqueza visual, animaciones)
- Prioridad  : alta (pedido directo de Esteban, 2026-07-09)
- Estado     : pendiente de análisis (el ADR es el entregable; nada se implementa sin él)
- Objetivo   : escribir el ADR que define el sistema visual nuevo SOBRE la fundación del ADR 031: (1) **superficie y elevación**: evolución de las cards planas a profundidad ligera (escala de 2-3 niveles de sombra sutil, radios, aire, separación de bloques) con tokens `--fk-*` y presupuesto de costo por regla; (2) **color secundario por dominio** (extensión del ADR 031: hoy hay principal/-bg/-text; decidir la rampa secundaria y sus usos); (3) **riqueza visual**: degradados suaves tokenizados, formas orgánicas SVG estáticas de fondo, patrones discretos e ilustraciones como clase nueva de asset del pipeline del ADR 026 (Esteban diseña; el ADR define dónde y cuántas); (4) **catálogo de animaciones con propósito** (aparición de tarjetas, completar meta, registrar gasto, pagar deuda; 150-250 ms, `prefers-reduced-motion`, coordinado con LG.2 para no duplicar celebraciones); (5) **resolución formal de la tensión de iconografía** ("familia por sección" vs lenguaje único del ADR 023); (6) **guardarraíles duros**: ambos temas (las referencias son claras; Finko es oscuro por defecto), WCAG AA verificado con cálculo real (método IV.1), Lighthouse 100 se mantiene, `pnpm perf` sin regresión, prohibidos backdrop-filter/blurs/transparencias costosas/animaciones permanentes (el brief mismo los veta). Entregable: ADR + plan de rebanadas verificables (DV.2a tokens de superficie, DV.2b riqueza por sección piloto, etc.) que las iniciativas v2 consumen.
- Secciones  : Transversal (`styles/tokens.css` y todo el sistema de diseño; `docs/DESIGN_SYSTEM.md`)
- Archivos   : punto de partida `styles/tokens.css`, `styles/components/atoms.css`, [ADR 031](DECISIONS/031-identidad-de-color-por-seccion.md), [ADR 023](DECISIONS/023-lenguaje-de-iconografia-propio.md)/[025](DECISIONS/025-logotipos-de-marca-y-tejas.md)/[026](DECISIONS/026-biblioteca-de-recursos-graficos.md)/[027](DECISIONS/027-logos-de-marca-a-color-excepcion-monocromo.md) (vigentes, se extienden sin revertir)
- Depende de : recomendado tras IV.2 (o en paralelo: DV.1 es análisis sin código); las ilustraciones/formas definitivas son cola de diseño de Esteban (ADR 026), no bloquean el ADR
- Modelo     : Fable 5 - Alto (sistema de diseño completo con trade-offs reales entre estética, rendimiento, accesibilidad y 2 temas; toca 5 ADRs vigentes)
...
- Estado     : **IV.2a cerrada 2026-07-09** (nav sidebar/bottom-nav + pestañas del hub Ahorros teñidas por dominio; teja de icono+acento en los 11 encabezados de sección). **IV.2d cerrada 2026-07-10** (migración `color: var(--fk-dom-X)` → `-text` en Análisis, modal Registrar y `.nudge-high`/`.dom-badge--*`). **Franja de modales cerrada 2026-07-10** (encontrada como WIP sin commitear de una sesión anterior, verificada en el navegador en ambos temas y cerrada junto con IV.2d): los 15 modales de registro con dominio propio llevan `data-dom` y una franja superior de 3px en el color crudo del dominio (`styles/modals.css`, `index.html`); los que no son de un dominio (import, legal, registrar, más, personalizar accesos, instalar iOS, onboarding) quedan correctamente sin franja. Ver CHANGELOG. Siguen IV.2b (acotada ahora solo a barras/anillos de progreso) e IV.2c.
- Objetivo   : **IV.2b (restante):** barras/anillos de progreso en el color del dominio (`.progress-bar`/`.progress-ring-wrap` en `atoms.css` siguen en `--fk-accent`/semánticos genéricos, sin distinguir dominio; decidir qué dominio aplica a cada vista: Metas/Fondo/Apartados/Inversión/Presupuesto). **IV.2c:** `.cal-dot--fijo` de amarillo a índigo; completar tejas/badges en Inicio y Movimientos donde falten (`.cal-dot--*`/`.cal-detail__icon--*` de Calendario y `.vencidos-card__icon--*`/`.prioridades-card__dot` de Inicio quedaron deliberadamente sin migrar en IV.2d: viven en el alcance de IV.2c, que probablemente rediseñe ese markup).
- **Hallazgo de IV.2a, aplica directo a la calibración de opacidad de IV.2c:** medido con la fórmula WCAG real (no a ojo), un tinte de 12% (`--fk-dom-X-bg`) usado como fondo de un elemento que además lleva `-text` como color de texto encima cae a 4.22-4.46:1 en tema claro para varios dominios (por debajo de 4.5:1, AA texto). 6% sí cumple (peor caso 4.54:1). La distinción real: **si el contenido sobre el tinte es texto real, el umbral es 4.5:1 (WCAG 1.4.3) y el tinte debe bajar a ~6%; si es un glifo/icono decorativo, el umbral es 3:1 (WCAG 1.4.11) y 12-14% está de sobra.** IV.2c debe aplicar esta distinción al decidir la opacidad de las tarjetas de evento del calendario (Esteban pidió 5-10%): si llevan texto encima del tinte, usar ~6-8%, verificado con cálculo real antes de fijarlo, no una sola opacidad "para toda la app" sin distinguir el caso.
- **Specs integradas por triaje 2026-07-08 (briefs de Inicio y Calendario):** (1) IV.2c: las tarjetas de evento del calendario abandonan la línea lateral de color y pasan a fondo teñido de baja opacidad del dominio del evento (ver hallazgo de arriba para la opacidad correcta). (2) IV.2c: en "Pendientes del mes" y "Próximas prioridades" de Inicio, cada ítem lleva el icono de su categoría + la teja/color de su sección de origen (Deudas, Calendario, Apartados, Metas, fijos), para reconocer el origen sin leer; **ampliado por el 3.er lote (2026-07-08):** además del icono y el color, cada ítem lleva una **etiqueta pequeña con el tipo** ("Deuda", "Gasto fijo", "Meta"...), cumpliendo la regla "el color nunca viaja solo" del ADR 031 D1. Nota: los colores de los emojis del brief (🟥🟧🟩🟦🟪) son ilustrativos; mandan los tokens `--fk-dom-*` aprobados. Los logos de marca en eventos ya existen (MK.2/`tejaMarca`); su evolución es el ADR 029, no esta tarjeta.
- Secciones  : Transversal (todas las vistas, solo CSS + atributos `data-*` existentes)
- Depende de : nada (IV.1 e IV.2a cerradas)
