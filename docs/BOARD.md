...
- **Rebanadas (ADR 041):** **MC.13a** motor mitad A (`ocurrenciasEnRango`/`ventanaDelCobro`) + Agenda consume → **MC.13b** motor mitad B (aportes por período) + Metas/Apartados borran sus copias → **MC.13c** `obligacionesYAportesDelCobro` + checklist consume (absorbe MC.7g) → **MC.13d** schema `cuentaId` en Ingreso (bump v26→v27) + form (independiente) → **MC.13e+** asistente v2 UI (puntos 9-20; depende de la decisión (a) para el flujo esporádico).
