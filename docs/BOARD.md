...
> Última actualización: 2026-07-13 (D.15d re-cortada; D.15d-1 cierra el motor puro + extracción de estimarSalarioMensual a infra; D.15d-2 (vista) es la siguiente).

---

## En proceso

_(Sin tarjeta activa: **D.15d-1** (motor puro `recomendarPalanca` + extracción de `estimarSalarioMensual` a `infra/financiero.js`) cerró el 2026-07-13, primera de las dos rebanadas en que se re-cortó D.15d (regla 2.1 + precedente MC.17a: lógica pura antes que su consumidor). El rediseño visual "Deudas v2" (**D.16a a D.16d**, [ADR 036](DECISIONS/036-deudas-v2-visual.md)) cerró completo el 2026-07-12. El [ADR 033](DECISIONS/033-direccion-visual-premium.md) sigue en "Propuesta con estreno parcial autorizado" (D1/D2, ahora con tres consumidores: Inicio, Mis cuentas y Deudas); **DV.2a** (despliegue global) sigue bloqueada hasta la validación completa de P1-P5. Siguiente tarjeta sugerida: **D.15d-2** (la vista que consume el motor y saca las 3 palancas a primer plano), abajo en Deudas.)_
...
> **D.16 - Rediseño visual de la sección ([ADR 036](DECISIONS/036-deudas-v2-visual.md)), CERRADA COMPLETA el 2026-07-12**: handoff de Claude Design del 2026-07-12 (mockup `Deudas v2.dc.html` + `SCREENS/deudas-v2.md`, bundle "Iteración de specimen"), enviado por Esteban con instrucción de implementar; mismo rol que MC.18/ADR 035 tuvo en Mis cuentas. **Absorbió D.15c** (la tarjeta de deuda con jerarquía visual es el D5 del handoff) y la mitad visual de D.15a (alerta de 2 capas; D.15a re-cortada abajo a solo copy). Orden final de la pantalla: hero total de deuda → card de estrategia → lista de deudas en tarjetas. Las 4 rebanadas cerradas (ver CHANGELOG): **D.16a** (hero + ojo), **D.16b** (picker con identidad de sección + comparativa como callout + drafts `i-snowball`/`i-handshake`/`i-trending-down`), **D.16c** (acelerador + panel inviable en 2 capas + fix del `btn--primary` inexistente), **D.16d** (tarjeta de deuda con chips + máscara + empty state). Cero cambios de lógica (BUG-011 intacto). Preguntas abiertas no bloqueantes del ADR 036: cifra protagonista del hero (saldo vs cuota), posición de "Saldada" en el orden, y las metáforas de íconos que viven en IV.4. Las piezas funcionales de la iniciativa Deudas v2 siguen abajo: D.15a (copy), D.15b (editar), **D.15d (motor de palanca, la siguiente)** y D.15e.
...
> **D.16d cerrada el 2026-07-12** (tarjeta de deuda con chips + máscara + empty state, ver CHANGELOG): cierra **D.16 completa** y absorbe D.15c. `.deuda-card` con teja 44px + badge de orden superpuesto, saldo prominente enmascarable, chips de categoría/tasa, callout ámbar de tasa, Abonar con tinte de compromisos, encabezado "Tus deudas · Orden X". 7 tests unitarios nuevos, gate final 179/179 E2E, SW v370.

> **D.15 - Análisis + re-corte hechos el 2026-07-12** (Opus 4.8, ver bloque de diseño en [`contexto/deudas.md`](contexto/deudas.md)). Hallazgos clave: (a) el **motor de orden de pago** (Avalancha vs Bola, `recomendarEstrategia`) ya existe y está probado, no se rehace; (b) las **3 macro-acciones** (Aumentar/Renegociar/Consolidar) ya están implementadas pero **enterradas** en el panel "plan inviable": el usuario con plan viable nunca las ve; (c) **editar deuda** el flujo existe (`_editarCompromiso` + `renderFormDeuda(tipo, deuda)`) pero **la tarjeta de deuda no tiene botón de editar** (solo abonar/archivar/eliminar); el único acceso hoy es desde Calendario, para fijos; (d) **"Aplicar aumento"** ya existe en el panel inviable pero **no en el acelerador del plan viable**. El brief pide una **capa nueva de recomendación de palanca** ortogonal al motor de orden: la tarjeta pasa a 2 niveles (orden Avalancha/Bola, sin cambio; palanca Aumentar/Renegociar/Consolidar, sacada a primer plano + principal recomendada). **Decisiones de Esteban (2026-07-12):** capacidad de pago = **margen libre real** (`ingresoMensual − fijosMensuales − Σ cuotas de deuda`); si el margen es positivo → recomienda Aumentar; sin margen + tasa alta → Renegociar; varias deudas costosas → Consolidar. **Fuente de la capacidad (arquitectura):** extraer `estimarSalarioMensual` a `infra/financiero.js` (hoy vive en `tesoreria/logic/ingresos.js`; `presupuesto/view.js` ya lo importa cruzando dominio; con `compromisos` sería el 3.º consumidor → hogar único en `infra/`, ADN 10 limpio). El motor queda **puro**: recibe `ingresoMensual`/`fijosMensuales` como parámetros. Ninguna rebanada revisa el ADR 019 (esa nota era para LIM.1). Re-corte en 5 rebanadas abajo. **Orden actualizado el 2026-07-12:** Esteban envió el handoff visual (D.16, [ADR 036](DECISIONS/036-deudas-v2-visual.md)) con instrucción de implementarlo primero; **D.15d pasa a ser la siguiente rebanada funcional después de D.16a-d** y construirá sobre los componentes visuales nuevos. **Re-corte del 2026-07-13 (triaje, regla 2.1 + precedente MC.17a):** D.15d tocaba infra + lógica pura + vista, así que se partió en **D.15d-1** (lógica sin UI: extracción a infra + motor puro, CERRADA el 2026-07-13, verificable por tests) y **D.15d-2** (la vista que consume el motor, verificable en la app).

> **D.15d-1 CERRADA el 2026-07-13** (motor puro + extracción, ver CHANGELOG y [`contexto/deudas.md`](contexto/deudas.md)): `recomendarPalanca(deudas, { ingresoMensual, fijosMensuales })` en `logic/estrategia.js` (palanca principal por margen libre real; retorna `{ principal, orden, capacidad, tieneCapacidad, razon }`; umbrales `TASA_ALTA_EA` 0.25 y `UMBRAL_CAPACIDAD_MINIMA` 20.000; puro, sin `S` ni `tesoreria`). `estimarSalarioMensual` movida de `tesoreria/logic/ingresos.js` a `infra/financiero.js` (barrel de tesorería re-exporta; presupuesto/acciones/distribucion repuntados a infra). 12 tests unitarios nuevos, sin consumidor todavía (eso es D.15d-2). SW v370→v371.

#### D.15d-2 - Deudas v2: sacar las 3 palancas a primer plano en la vista (consume el motor)
- Prioridad  : alta (corazón de la iniciativa, ahora visible)
- Estado     : lista para iniciar (el motor `recomendarPalanca` ya existe y está probado)
- Objetivo   : (puntos 2+3 del brief) las 3 macro-acciones (Aumentar la cuota / Renegociar la tasa / Consolidar) hoy solo se ven enterradas dentro del panel "plan inviable" (`_renderPanelAlternativas`); un usuario con plan viable nunca las descubre. Sacarlas a **primer plano SIEMPRE**, con intro breve y **pesos visuales distintos** según el `orden` que devuelve el motor (la principal recomendada destacada, no 3 botones iguales). La vista calcula la capacidad: importa `estimarSalarioMensual` de `infra/financiero.js` (ADN 10) para `ingresoMensual` y suma los compromisos tipo 'fijo' con las funciones del propio dominio (`calcularCompromisoMensual`/helper de fijos) para `fijosMensuales`, y llama `recomendarPalanca(deudas, { ingresoMensual, fijosMensuales })`. Reutiliza los renderers existentes `renderRenegociar`/`renderConsolidar` (`estrategia-impacto.js`) y el remedio de aumento; coordina con los componentes visuales de D.16 (card de estrategia con identidad de sección, tiles, tinte frambuesa).
- **Cuidado obligatorio (BUG-011):** la ESTRUCTURA de la card se decide solo con datos registrados; el extra simulado nunca decide estructura (ver riesgo "estado UI simulado" en la ficha). Las palancas a primer plano NO deben reintroducir esa dependencia. Suites de regresión: `describe "BUG-011"` en unit + suite "BUG-011" en `tests/e2e/estrategia-pago.test.js`.
- Secciones  : Deudas (`compromisos/views/estrategia.js`, `views/estrategia-impacto.js`, `index.js` para el wiring de selección), CSS de la card
- Archivos   : `modules/dominio/compromisos/views/estrategia.js`, `modules/dominio/compromisos/views/estrategia-impacto.js`, `modules/dominio/compromisos/index.js`, `styles/components/charts.css` (o donde vivan `.estrategia-card__*`), un helper de fijos mensuales en `logic/modelo.js` si conviene, tests unit + E2E
- Depende de : D.15d-1 (motor, ya cerrado); coordina con D.15b (el trigger de editar en la `.deuda-card`) solo si tocan el mismo markup
- Modelo     : Opus 4.8 - Alto (rediseño de la card de estrategia preservando el invariante de BUG-011 + integración del motor en la vista)
...
- Objetivo   : (punto 12) botón "Aplicar" en el acelerador "¿Puedes pagar más rápido?" del plan viable (hoy tiene input sin Aplicar; el panel inviable ya lo tiene). Reusa `repartirExtraEnCuotas()` (ya probado) para convertir el extra simulado en cuotas nuevas con confirmación, sin editar a mano.
- Secciones  : Deudas (`compromisos/views/estrategia.js` `_renderAceleradorExtra`, `index.js` `aplicar-aumento-cuota`)
- Depende de : nada
- Modelo     : Sonnet 5 - Bajo (botón + reuso de lógica existente)
