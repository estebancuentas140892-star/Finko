...
- Abierto por 09: **R89** ("toda lista de más de dos elementos tiene un orden, y lo dice") llega a la **ficha 18** con una pregunta de alcance: la 04 es coherencia entre vistas y la 05 y la 09 son ordenación dentro de una vista, así que hay que decidir si es una regla o dos. Y **R87 no se consolida** hasta que la 18 le escriba la cláusula del movimiento que apunta a un objeto concreto de una lista (la ficha 09 invirtió su reparto). La **ficha 10** hereda la pieza `ordenarBolsasPorFecha()`: si a Reservas también le falta orden, R89 gana su cuarto caso.
