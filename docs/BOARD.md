...
- Prioridad  : alta
- Estado     : **análisis y diseño hechos el 2026-07-14 (Opus 4.8), [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md) en estado Propuesta**. Re-cortada en rebanadas MC.13a-e (abajo). **El motor (MC.13a-c) no depende de los conflictos y puede arrancar apenas Esteban apruebe el ADR 041; dos decisiones de Esteban bloquean rebanadas concretas** (ver "Decisiones pendientes" abajo y en el ADR). No iniciar ninguna rebanada sin la aprobación del ADR.
...
- **Rebanadas (ADR 041):** **MC.13a cerrada el 2026-07-14** (`infra/vencimientos.js` mitad A: `ocurrenciasEnMes`/`ocurrenciasEnRango`/`ventanaDelCobro` + Agenda lo consume, ver CHANGELOG y [`contexto/mis-cuentas.md`](contexto/mis-cuentas.md)) → **MC.13b** motor mitad B (aportes por período) + Metas/Apartados borran sus copias → **MC.13c** `obligacionesYAportesDelCobro` + checklist consume (absorbe MC.7g) → **MC.13d** schema `cuentaId` en Ingreso (bump v26→v27) + form (independiente) → **MC.13e+** asistente v2 UI (puntos 9-20; depende de la decisión (a) para el flujo esporádico).
...
- Depende de : **aprobación del [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md)** (análisis ya hecho); coordinar con PA.1 (conflicto (b)); decisión del conflicto (a) sobre NAV.A2b antes de MC.13e
- Modelo     : análisis/re-corte hecho con Opus 4.8 (ADR 041); rebanadas MC.13a-c Opus 4.8 - Alto (lógica de fechas/frecuencias), MC.13d Opus 4.8 - Extra (bump con migración), MC.13e+ Sonnet 5 - Alto por rebanada de UI
