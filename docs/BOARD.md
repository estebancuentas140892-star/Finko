...
>
> **Cuatro hallazgos cuestionan una decisión vigente y NO se ejecutan sin la palabra de Esteban** (regla 2.7, no se revierte un ADR en silencio): ~~MOV.1 vs TX.8b~~ (**aprobado y cerrado el 2026-07-22**), CAL.5 vs PA.1, la distribución de un toque vs MC.13e-2g, y MC.17f vs "MC.17 cerrada completa". Cada tarjeta lo dice en su Estado.
>
...
#### CAL.5 - Pagar en lote lo que vence hoy (un toque en vez de uno por uno)
- Prioridad  : media-alta
- Estado     : pendiente de análisis. **Necesita una decisión de Esteban antes de codificar (regla 2.7): ¿el lote manual va ANTES que PA.1?** La auditoría sostiene que sí: un batch de un toque captura la mayor parte del valor percibido de los pagos automáticos con una fracción del riesgo, porque el usuario sigue confirmando (no choca con "Finko refleja la realidad, no la inventa", la tensión que obligó a PA.1 a pedir ADR). Si Esteban prefiere ir directo a PA.1, esta tarjeta se absorbe ahí.
- Objetivo   : hoy pagar 6 gastos fijos son ~30 toques, porque cada uno pide cuenta por separado. Ofrecer una acción de lote desde el Calendario (y desde el bloque de vencidos de Inicio) que liste lo que vence hoy o está vencido, con todo marcado por defecto, resuelva la cuenta **una sola vez** para el grupo y registre los pagos juntos. Reutiliza `resolverPagoConSelector`/`distribuir-pago.js` (ya resuelven cuenta preferida y reparto sin dejar negativos) y el motor de vencimientos (`ocurrenciasEnMes`, `obligacionesYAportesDelCobro`), no construye un segundo motor. **Ojo (BUG-015):** la fecha de cada pago debe seguir la regla ya fijada (mes en curso → hoy; mes pasado → la ocurrencia de ese mes; futuro → no se paga).
- Secciones  : Calendario, Inicio (vencidos), Mis cuentas (descuento)
- Archivos   : `modules/dominio/agenda/` (vista del lote + handler), `modules/infra/distribuir-pago.js`, `modules/infra/vencimientos.js` (consumo)
- Depende de : la decisión de secuencia vs **PA.1**; conviene reusar el helper de "registrar pago de compromiso" de **ARQ.2** para no triplicar la aritmética una cuarta vez
- Modelo     : Opus 4.8 - Alto (dinero real en lote, con casos borde de saldo insuficiente y pagos parciales del grupo)
...
> **MC.13e-2a CERRADA el 2026-07-15** (copy, accesos cruzados y navegación del asistente, ver CHANGELOG y [`contexto/mis-cuentas.md`](contexto/mis-cuentas.md)): (11) el array `ctas` ("Ver progreso del fondo", "Explorar/Aportar a inversiones", "Ver estrategia de deudas", "Ver tu seguimiento en Límites de gasto") deja de renderizarse en la tarjeta compacta; `sugerirDistribucionIngreso()` lo sigue calculando intacto (sus tests no cambian), MC.13e-2g decide si vuelve, contextual a cada paso. (12) el aviso "recibiste tu ingreso" pasa a un bloque `.distribuir-aviso` separado, antes de `.distribuir-card`, que ahora siempre abre con la invitación genérica. (13) el `<legend>` del panel se retocó ("hacia" → "entre", "El resto" → "Lo que no distribuyas"). (17) los botones Atrás/Siguiente/Distribuir ganan los íconos del sprite (`i-chevron-right`, rotado 180° para "Atrás" por no existir una variante izquierda propia; `i-check-circle` en "Distribuir"), en vez de flechas de texto plano.
...
> **Iniciativa "Deudas v2: de registro a asesor" (brief de Esteban del 2026-07-08, 15 puntos) y su rediseño visual "Deudas v2" ([ADR 036](DECISIONS/036-deudas-v2-visual.md)) CERRADOS COMPLETOS el 2026-07-13.** Las 9 rebanadas: **D.14** (2026-07-10, acreditar cuenta de origen), **D.16a-d** (2026-07-12, rediseño visual completo: hero, picker/comparativa, acelerador/panel en 2 capas, tarjeta de deuda), **D.15c** (absorbida por D.16d), **D.15d-1/D.15d-2** (2026-07-13, motor puro `recomendarPalanca` + vista de las 3 palancas siempre visibles, absorbe D.15e), **D.15b** (2026-07-13, editar deuda + reorden del form) y **D.15a** (2026-07-13, copy de simulaciones + refuerzo en Abonar, última pieza). Historial completo de cada rebanada en [`CHANGELOG.md`](CHANGELOG.md) y en la ficha [`contexto/deudas.md`](contexto/deudas.md). Sin tarjetas pendientes en esta sección.)_
...
> **AP.5a cerrada el 2026-07-22** (quick win de la auditoría de UX/producto, patrón P1, ver CHANGELOG y [`contexto/apartados.md`](contexto/apartados.md)): al pulsar **Aportar**, el monto llega prellenado con el aporte sugerido (`calcularAporteSugerido`, mismo cálculo que ya alimentaba el hint en vivo del formulario de creación), editable siempre. Sin fecha objetivo o apartado ya cubierto, el campo sigue vacío. Ejecutada junto con **AH.5a** (Ahorro, misma rebanada, mismo patrón).
...
> **AH.5a cerrada el 2026-07-22** (quick win de la auditoría de UX/producto, patrón P1, ver CHANGELOG y [`contexto/ahorro.md`](contexto/ahorro.md)): "Registrar aporte" prellena el monto con la sugerencia de AH.2 (mismo cálculo que ya alimentaba la caja de sugerencia del compromiso mensual), editable siempre, sin perder la explicación de que el aporte no descuenta cuentas (ADR 009). Sin gastos fijos registrados (`objetivo <= 0`) el campo sigue vacío. Ejecutada junto con **AP.5a** (Apartados, misma rebanada, mismo patrón).
...
> **PE.7 CERRADA el 2026-07-22** ("Me deben" conectado a cuentas y patrimonio, ver CHANGELOG y la ficha nueva [`contexto/me-deben.md`](contexto/me-deben.md)): `Personal.cuentaId` opcional sin backfill ni bump; prestar descuenta la cuenta elegida, cobrar acredita `desglose.aplicado`, y el capital pendiente entra al patrimonio como bucket **"Por cobrar"**. **Decisión financiera clave:** solo suman como activo los préstamos **con `cuentaId`** (los que de verdad sacaron dinero de una cuenta); los que no, seguirían contados dentro de `cuentas` y se duplicarían, exactamente el criterio que ya excluye al fondo de emergencia. El interés pendiente NO cuenta (no se ha ganado ni cobrado). Invariante verificado en la app: prestar $400.000 baja el saldo pero **no mueve el patrimonio neto**. Borrar un préstamo **no** revierte los movimientos (divergencia deliberada de D.14, explicada en la ficha; se reevalúa al cerrar PE.6b, que traerá el historial de abonos). De paso se corrigieron dos defectos: el form se inyectaba una sola vez en el init (crear una cuenta después dejaba el selector invisible) y `resetModal()` borraba la preselección de cuenta y la fecha del préstamo.
...
> **CAT.1a CERRADA el 2026-07-13** (Gastos, ver CHANGELOG y [`contexto/transversal.md`](contexto/transversal.md)): `CATEGORIAS_GASTO_USUARIO` (`constants.js`) excluye ahora Vivienda y Servicios públicos, además de las ya excluidas (Deudas, Ahorro, Alimentación); `CATEGORIAS_GASTO` (catálogo base) las conserva intactas para que `CATEGORIA_ICONO` y la validación de límites sigan resolviendo bien los registros existentes (mismo precedente que "Alimentación" v15, sin bump de schema). El hint "normalmente pertenece a fijos" se retiró por completo: `CATEGORIAS_TIPICAMENTE_FIJAS` eliminada de `constants.js`, el `<p id="hint-categoria-fija">` eliminado de `renderFormGasto()` (`gastos/view.js`) y su listener de `change` eliminado de `_montarFormGasto()` (`gastos/index.js`). 4 tests unitarios nuevos + 2 tests viejos actualizados (afirmaban lo contrario) en `gastos.test.js`. 2600/2600 unit + 192/192 E2E completos + lint verdes. SW v380→v381.

> **CAT.1b CERRADA el 2026-07-13** (Apartados, ver CHANGELOG y [`contexto/transversal.md`](contexto/transversal.md)): `PLANTILLAS_APARTADO` (`apartados/logic.js`) pasa de 17 a 20 plantillas. Sale **Vacaciones** (ya vive en `CATEGORIAS_META`). "Matrícula o semestre" se divide: la plantilla queda como **"Matrícula escolar"** 🎓 (colegio, esporádico); el semestre universitario se planea como Meta. "Útiles escolares" se amplía a **"Útiles y uniformes"** 🎒. Entran **Veterinario** 🩺, **Mantenimiento del hogar** 🛠️, **Seguro del hogar** 🛡️ y **Reparaciones inesperadas** 🧰. Sin cambios en `_aplicarPlantilla()`/`renderFormApartado()`: el mecanismo de plantillas (`data-action="apartado-plantilla"` + `setIconoPickerValor`) es agnóstico al contenido del catálogo. 6 tests unitarios actualizados/nuevos en `apartados.test.js` (conteo 17→20, exclusión de Vacaciones/Matrícula o semestre/Útiles escolares, presencia de las 4 nuevas). Apartados existentes creados desde plantillas retiradas o renombradas conservan su `nombre`/`icono` guardado sin cambios (el catálogo solo afecta la creación de apartados nuevos). 2604/2604 unit + 192/192 E2E completos + lint verdes. SW v381→v382.
...
> **CAT.2f CERRADA el 2026-07-13** (Gasto fijo/Calendario, ver CHANGELOG y [`contexto/transversal.md`](contexto/transversal.md)): categoría "Otro". Análisis previo en la misma sesión (triaje 2.7): pregunta directa a Esteban sobre alcance, decisión de alcance mínimo (mismo patrón que CAT.2d/2e, sin esperar CAT.1). `agenda/view.js`: `renderFormGastoFijo()` agrega el grupo del picker, oculto por defecto; `_renderDetalleItem()` resuelve `c.icono` antes que `CATEGORIA_AGENDA_ICONO`. `agenda/index.js`: `_syncCategoriaGastoFijo()` (ya era el listener de `change` de AG.4) extendido para alternar la visibilidad del ícono; `setIconoPickerValor` prellena en edición. `compromisos/logic/modelo.js`: `normalizarCompromiso()` gana `base.icono` en la rama `tipo==='fijo'` (antes solo existía para deudas). `gastos/logic.js`: `iconoPorOrigen()` (TX.6/TX.7) hereda el ícono elegido en el gasto generado al pagar el fijo. El checklist de "Distribuir mi ingreso" no necesitó cambios (ya generalizado desde CAT.2d). 15 tests unitarios nuevos (`compromisos.test.js` + `agenda.test.js` + `gastos.test.js`) + 2 E2E nuevos. 2598/2598 unit + 192/192 E2E completos + lint verdes. SW v379→v380. **Cierra la iniciativa CAT.2 completa (CAT.2a-f).**
...
> **Iniciativa FORM: Formularios v2, lenguaje de captura compartido, CERRADA COMPLETA el 2026-07-15** ([ADR 042](DECISIONS/042-formularios-v2-visual.md), aceptado 2026-07-15; handoff de Claude Design "Iteración de specimen", enviado por Esteban con instrucción de implementar). Octava entrega de la familia visual v2 y fuente única del lenguaje de formularios: monto hero protagonista, categoría con chips de ícono (nunca un select nuevo), fecha con atajos Hoy/Ayer/Otra fecha, teja de dominio en el header del modal, footer con primario a lo ancho. Las 3 rebanadas: **FORM.1a** (fundación CSS en `forms.css` + Registrar gasto flagship, revisó el orden de TX.9a y corrigió de paso el hueco móvil de `responsive.css` sobre `.input--big-amount`), **FORM.1b** (Nueva deuda: segmented inline, chips 2col, monto hero, disclosure de tasa, toggle D.14, retiro completo del chooser de dos pasos; el toggle destapó que la app ya tenía dos switches, consolidados en **TX.11**) y **FORM.1c** (Nuevo gasto fijo: chips 3col, monto hero, frecuencia+día en fila, banner informativo dinámico). Historial completo de cada rebanada en CHANGELOG y en las fichas [`contexto/transversal.md`](contexto/transversal.md), [`contexto/deudas.md`](contexto/deudas.md) y [`contexto/calendario.md`](contexto/calendario.md). Los demás formularios de la app migran cuando su iniciativa los toque (ADR 042 D6), NO aquí. **Conflicto señalado (D9), sigue sin resolver:** AP.5 pedía dropdown de categoría; la decisión es de Esteban al iniciar esa tarjeta. Sin tarjetas pendientes en esta iniciativa.

> **TX.11 CERRADA el 2026-07-15** (un solo switch para toda la app, ver CHANGELOG y [`contexto/configuracion.md`](contexto/configuracion.md)/[`contexto/deudas.md`](contexto/deudas.md)): de los tres switches que tenía la app, `.toggle` de `atoms.css` (sin consumidores) queda como el único, con `var(--fk-section-accent, var(--fk-accent))` para heredar el tinte de dominio. `.config-toggle` (Ajustes) y `.toggle-row` (formularios) se conservan como los layouts que lo envuelven; `.config-toggle input[type="checkbox"]*` y `.toggle-switch*` se eliminan del todo. Migrados sus dos consumidores vivos (tema/notificaciones en Ajustes, D.14 en Deudas), sin tocar wiring por `id`.
...
#### EDIT.1 - Editar sin destruir: Metas, Apartados, Inversión y Me deben
- Prioridad  : media-alta
- Estado     : pendiente de análisis. Hallazgo de la auditoría de UX/producto (2026-07-21), patrón P3.
- Objetivo   : cuatro secciones no permiten **editar** lo ya creado: corregir un nombre mal escrito, un objetivo, una fecha o una tasa obliga a **eliminar y recrear**, perdiendo en el camino el progreso, los aportes y los intereses acumulados. Es destrucción de datos como precio de una corrección tipográfica. Solo el fondo, las cuentas, los ingresos y las deudas se editan hoy. Aplicar el patrón que **D.15b** ya estableció para Deudas (editar reusando el mismo formulario, prellenado, sin tocar el histórico) a Metas, Apartados, Inversión y Me deben, en una rebanada por sección para poder verificar cada una. **Ojo del análisis:** editar el objetivo de una bolsa con aportes ya hechos cambia su progreso; decidir si se conserva el histórico tal cual (recomendado) o se recalcula, y dejarlo escrito.
- Secciones  : Metas, Apartados, Inversión, Me deben
- Archivos   : `modules/dominio/metas/`, `apartados/`, `inversiones/`, `personales/` (form + acciones de cada uno), patrón de referencia en `compromisos` (D.15b)
- Depende de : nada duro. Coordina con **MOV.1** (el ledger accionable necesita que el dominio dueño sepa editar) y con **ARQ.1** (si las 4 bolsas comparten componente, el formulario de edición se escribe una vez, no cuatro): decidir el orden al iniciar, porque hacerlo antes de ARQ.1 significa escribir cuatro editores y luego unificarlos
- Modelo     : Sonnet 5 - Alto por rebanada (patrón ya probado en D.15b; sin lógica financiera nueva salvo la decisión de progreso)
...
- Depende de : nada. Conviene **antes** de EDIT.1 (si no, se escriben cuatro editores y luego se unifican) y antes de que las iniciativas v2 de esas secciones rehagan sus vistas
...
- Depende de : nada; el helper de pago (2) conviene **antes** de CAL.5, que si no escribiría una cuarta copia
...
- **Cuestionada en la secuencia por el triaje de la auditoría (2026-07-21):** la auditoría coincide en que esta es la automatización de mayor impacto percibido ("que se pague solo") y también la más delicada, y propone probar **antes** el pago en lote manual de un toque (**CAL.5**, sección Calendario): captura la mayor parte del valor con una fracción del riesgo, porque el usuario sigue confirmando y no se toca la filosofía "Finko refleja la realidad, no la inventa". Sirve además de puente: si el lote se usa mucho, el ADR de PA.1 llega con evidencia real de uso en vez de intuición. **La secuencia es decisión de Esteban**: si prefiere ir directo al ADR, CAL.5 se absorbe aquí.
