...
> **D.15d-2 CERRADA el 2026-07-13** (vista de palancas, **absorbe D.15e**, ver CHANGELOG y [`contexto/deudas.md`](contexto/deudas.md)): la card de estrategia pasa a 2 niveles (orden Avalancha/Bola arriba; sección de palancas Aumentar/Renegociar/Consolidar SIEMPRE visible abajo, con la principal recomendada por `recomendarPalanca` y pesos visuales según `palanca.orden`). La vista calcula la capacidad con `estimarSalarioMensual` (infra) + `calcularFijosMensuales` (nueva en `modelo.js`). Retira el acelerador `<details>` (su input vive en la palanca "Aumentar" con "Aplicar" → **D.15e absorbida**); panel inviable en 2 capas puras (alarma = diagnóstico; solución = palancas). BUG-011 intacto (estructura con extra=0). Bloque unit de la vista reescrito + `describe D.15d-2`; E2E `estrategia-pago` reescrito (helper `elegirPalanca`). 2520 unit + 179/179 E2E + lint verdes. SW v371→v372.
