...
- **De DIS.19 (2026-07-29):** `diasHastaFecha` + `planDeReferencia` + `estadoDeBolsa` (`infra/bolsas.js`) y la cadena de proyección de Inversión (`infra/portafolio.js`) ya viven en infra con re-export; `tests/unit/bolsas.test.js` compara identidad de función para que una copia local nueva falle.
...
- Depende de : nada. ARQ.1a (cerrada) ya demostró que unificar después de escribir los 4 editores es viable
- Modelo     : Alta capacidad - Alto (lo que queda es más chico que antes de ARQ.1a: sin el riesgo de redondeo de `diasHastaFecha`, ya resuelto)
