...
- Estado     : **IV.2a cerrada 2026-07-09**. **IV.2d cerrada 2026-07-10** (migración `color: var(--fk-dom-X)` → `-text`). **IV.2b cerrada 2026-07-10**: franja de modales (encontrada como WIP sin commitear de una sesión anterior, verificada) + barras/anillos de progreso de Metas, Ahorro/fondo, Apartados y Me deben, y los factores Deuda/Liquidez/Ahorro del score de Análisis, todos vía la variable compartida `--fk-section-accent` (heredada de `[data-dom="X"]`, `styles/modals.css`) leída por `.progress-bar`/`.progress-ring-wrap` (`atoms.css`) solo en su estado por defecto (`--near`/`--complete` siguen semánticos, sin tocar). **Presupuesto/Límites (`_renderGrupoCard`) quedó fuera a propósito**: ya tiene un esquema de color propio por ADR 019 (Necesidades neutro, Ahorro celebra, Estilo de vida alerta) que un tinte de dominio rompería (Necesidades dejaría de verse neutral). Pendiente de decisión, coincide con LIM.1. El factor "Control" del score de Análisis tampoco se tiñó (mide volatilidad de gasto, no un dominio de la app). Ver CHANGELOG. Sigue IV.2c.
- Objetivo   : **IV.2c (restante):** `.cal-dot--fijo` de amarillo a índigo; completar tejas/badges en Inicio y Movimientos donde falten (`.cal-dot--*`/`.cal-detail__icon--*` de Calendario y `.vencidos-card__icon--*`/`.prioridades-card__dot` de Inicio quedaron deliberadamente sin migrar en IV.2d: viven en el alcance de IV.2c, que probablemente rediseñe ese markup).
