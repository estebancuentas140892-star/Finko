...
> Última actualización: 2026-07-15 (TX.11 cerrada → un solo switch para toda la app, consolidando los tres que dejó al descubierto la verificación de FORM.1b. Con esto, la iniciativa Formularios v2 ([ADR 042](DECISIONS/042-formularios-v2-visual.md)) y todos sus hallazgos derivados quedan completos. MC.13: sólo falta el asistente v2 UI, MC.13e+, [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md)).
...
- Prioridad  : alta
- Estado     : **en curso. El motor está completo y en producción** (MC.13a-d + MC.13c-2, cerradas el 2026-07-14; [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md) aceptado parcialmente). **MC.13e-1 cerrada el 2026-07-15** (decisión (a) implementada). **Sigue MC.13e-2+**: el rediseño del asistente en sí (puntos 9-21 del brief), sin re-cortar todavía en rebanadas verificables (regla 2.1); ese es el primer paso al retomarla, no un re-corte hecho de antemano.
...
- **Rebanadas (ADR 041):** **MC.13a cerrada el 2026-07-14** (`infra/vencimientos.js` mitad A: `ocurrenciasEnMes`/`ocurrenciasEnRango`/`ventanaDelCobro` + Agenda lo consume), **MC.13b cerrada el 2026-07-14** (mitad B: `FRECUENCIAS_APORTE`/`frecuenciaPrincipalIngresos`/`normalizarFrecuenciaAporte`/`diasPorPeriodo`/`etiquetaPeriodo`/`aportePorPeriodo` + Metas y Apartados borran sus copias; cero tests existentes tocados) y **MC.13c-1 cerrada el 2026-07-14** (`obligacionesYAportesDelCobro` puro: compone `ventana`/`vencidas`/`enVentana`/`aportes`; 40 tests, sin consumidores todavía), **el motor está completo**, **MC.13d cerrada el 2026-07-14** (campo opcional `cuentaId` en `Ingreso`, bump v26→v27 con migración no-op + el form lo captura con el patrón 0/1/varias), **MC.13c-2 cerrada el 2026-07-14** (la checklist consume el motor: **MC.7g cerrado**, todas las frecuencias, "ya pagado" por ventana del cobro) y **MC.13e-1 cerrada el 2026-07-15** (decisión (a): un ingreso esporádico ya NO ofrece distribuirlo automáticamente; se eliminó el modo "ya acreditado" del asistente por completo, dead code tras quitar su único disparador; el asistente sigue accesible manualmente desde Mis cuentas, ver CHANGELOG y [`contexto/mis-cuentas.md`](contexto/mis-cuentas.md)) → **sigue MC.13e-2+**: el rediseño del asistente en sí (puntos 9-21). MC.13c-3 (datar todas las frecuencias) se separó abajo y no bloquea.
- **Ampliación del brief 2026-07-08 (21 puntos + ingreso fijo→cuenta), pendiente MC.13e-2+:** (9) rediseño en 2 pasos: primero recomendaciones visuales de cómo distribuyen los expertos (educación financiera con barras/gráficos/porcentajes; candidato a logro por distribución saludable sostenida), después la distribución personalizada por prioridad; (10) cada recomendación aparece SOLO en el paso de su categoría, no todas juntas al inicio; (11) quitar los accesos "Ver progreso del fondo / estrategias / límites" del asistente; (12) el aviso "recibiste tu ingreso" y el asistente son dos bloques visualmente independientes; (13) revisar el texto "Reparte hacia tus necesidades..."; (14) si el ingreso no alcanza, ofrecer completar con saldo disponible de otras cuentas (detección automática + pregunta explícita); (15) en cada fila de la distribución: logo de entidad o icono personalizado + nombre + nota (diferenciar dos "Arriendo" por icono/nota); (16) quitar "Abonar extra a deudas" del paso de ahorro (un abono es un pago, vive en Deudas); (17) navegación Atrás/Siguiente modernizada (coherente con IV); (18) el dinero restante exige decisión explícita (dejarlo en la cuenta, ahorro, meta) antes de finalizar; (21) metas/fondo/apartados muestran la **cuota del período** según la frecuencia del ingreso, no el objetivo total; **(integración ingreso fijo)** el ingreso fijo registra en qué cuenta se recibe, el asistente parte de esa cuenta, los pagos aprobados se descuentan de ahí, y el paso final "Estilo de vida" desaparece: lo no distribuido simplemente se queda en la cuenta (refleja la realidad; revisa el cierre del asistente MC.7e); **(7+19) CERRADO por MC.13e-1**: separación clara ingreso fijo (periódico, dispara distribución) vs ingreso esporádico (solo acredita y registra, sin ofrecer distribución).
...
- **Pieza de infraestructura compartida (regla de arquitecto 2.7): CONSTRUIDA** el 2026-07-14 en `modules/infra/vencimientos.js` (MC.13a + MC.13b). El "motor de vencimientos y aportes recomendados" (qué obligaciones y aportes corresponden a una fecha/ventana dada, y cuánto tocaría aportar según la frecuencia real de ingresos) ya existe y tiene sus dos mitades. Lo consumen hoy Agenda, Metas y Apartados; lo consumirán, importando en vez de copiar: este asistente, la checklist de Necesidades (ex MC.7g), los pagos automáticos (PA.1, Transversal), la cuota por período del punto 21, el **plan de aportes de Metas v2** (MT.6), el **prellenado del botón Aportar de Apartados** (AP.5) y el **aporte del fondo de emergencia en la distribución** (AH.5; AH.2 ya calcula el valor). **Regla vigente:** ningún dominio vuelve a escribir su propia tabla de frecuencias ni su propio reparto por período.
- Secciones  : Mis cuentas (`tesoreria/logic/distribucion.js`, `ingresos.js`), transversal por el motor (`infra/vencimientos.js` nuevo, `agenda`, `metas`, `apartados`)
- Depende de : **nada bloquea MC.13e-2+** (la decisión (a) ya está resuelta); coordinar con PA.1 (conflicto (b), independiente)
- Modelo     : análisis/re-corte hecho con Opus 4.8 (ADR 041); MC.13a, MC.13b, MC.13c-1 y MC.13d hechas con Opus 4.8; MC.13c-2 Opus 4.8 - Alto (semántica de pagos); MC.13e-1 Sonnet 5 - Alto; MC.13e-2+ empieza con su propio re-corte (Sonnet 5 - Alto por rebanada, sube a Opus si alguna toca lógica financiera no trivial)
