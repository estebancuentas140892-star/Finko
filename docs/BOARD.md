...
Las 56 tarjetas del tablero, para elegir la próxima sin cargar el archivo completo (principio 9). "Depende de" va acortado a la referencia clave; el texto completo vive en la tarjeta, más abajo por sección.

| ID | Título | Sección | Prioridad | Depende de |
|---|---|---|---|---|
| IN.9d | Accesos y actividad separados en escritorio, fila final 6+6 | Inicio | media | nada (ADR 057 D4 ya cubre) |
...
> **Iniciativa "IN.9 - Inicio en escritorio"** (auditoría de navegación global de Claude Design, 2026-07-31; handoff en `Auditoría navegación global Finko-handoff/`). Fuente única del reparto de Inicio sobre 1024px. Ocho decisiones ID1 a ID8: **ID8 es un no-op** (decide *no* agregar un botón de registrar, ya cubierto por ADR 024) y **ID4 e ID7 salen a INT.1**, porque dependen de una barra superior de escritorio que no existe en el repo. **IN.9c cerró** (columna propia del detalle por cuenta, D3 del ADR 057), quedan ID3 e ID6 en IN.9d e IN.9e (ID2 e ID5 ya cerraron). **IN.9d ya tiene ADR** (057 D4): el tablero la traía marcada "requiere ADR" por error, misma corrección que IN.9c. **PI10 sigue abierto:** un usuario que registra poco deja la columna de actividad corta y desbalancea la fila final; se decide en IN.9d, que es la que arma esa fila. El informe deja además tres reglas para el design system (R78 ancho completo que se gana, R79 gráfico dimensionado por su dato, R80 acordeón como respuesta al ancho escaso) y dos decisiones abiertas que viajan con INT.1: `--fk-bg-glass` sin valor en tema claro (PI7) y el contraste de los tokens `--fk-dom-*` (PI8, medido: 3,23:1 y 2,65:1, bajo el mínimo 3:1 de WCAG 1.4.11). **Móvil no cambia en ninguna rebanada salvo lo que IN.9a decida.**

#### IN.9d - Accesos y actividad separados en escritorio, fila final 6+6
- Prioridad  : media
- Estado     : pendiente. El ADR ya existe (057 D4, acota el 034 D7 a móvil, aceptado 2026-07-31); no bloquea, lista para ejecutar.
- Área       : ambos
...
- Depende de : nada (ADR 057 D4 ya cubre; IN.9c ya cerró, mismo marcado)
