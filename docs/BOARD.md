...
> Última actualización: 2026-07-14 (MC.13b cerrada → el motor de vencimientos y aportes queda completo, sigue MC.13c; ver [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md) y CHANGELOG).
...
- Prioridad  : alta
- Estado     : **en curso. El motor está completo y en producción** (MC.13a mitad A + MC.13b mitad B, cerradas el 2026-07-14; [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md) aceptado parcialmente). Sigue **MC.13c**, que ya tiene sus dos mitades listas. **Dos decisiones de Esteban siguen abiertas** (ver "Decisiones pendientes" abajo y en el ADR): no bloquean MC.13c ni MC.13d, sí MC.13e.
...
- **Decisiones pendientes de Esteban (regla 2.7):** (a) quitar la oferta automática de distribución tras un ingreso esporádico **revierte NAV.A2b s2 del ADR 024** (recomendación del análisis: quitar la oferta automática pero conservar el asistente accesible manualmente); (b) el crédito automático del ingreso fijo a la cuenta es materia de PA.1 (movimiento sin confirmación), fuera del alcance de este ADR, que solo agrega el dato `cuentaId`.
...
- **Rebanadas (ADR 041):** **MC.13a cerrada el 2026-07-14** (`infra/vencimientos.js` mitad A: `ocurrenciasEnMes`/`ocurrenciasEnRango`/`ventanaDelCobro` + Agenda lo consume), **MC.13b cerrada el 2026-07-14** (mitad B: `FRECUENCIAS_APORTE`/`frecuenciaPrincipalIngresos`/`normalizarFrecuenciaAporte`/`diasPorPeriodo`/`etiquetaPeriodo`/`aportePorPeriodo` + Metas y Apartados borran sus copias; cero tests existentes tocados) y **MC.13c-1 cerrada el 2026-07-14** (`obligacionesYAportesDelCobro` puro: compone `ventana`/`vencidas`/`enVentana`/`aportes`; 40 tests, sin consumidores todavía), **el motor está completo**, y **MC.13d cerrada el 2026-07-14** (campo opcional `cuentaId` en `Ingreso`, bump v26→v27 con migración no-op + el form lo captura con el patrón 0/1/varias), ver CHANGELOG y [`contexto/mis-cuentas.md`](contexto/mis-cuentas.md) → **MC.13c-2** (abajo: la checklist consume el motor, requiere 3 decisiones de Esteban) → **MC.13e+** asistente v2 UI (puntos 9-20; depende de la decisión (a) para el flujo esporádico y de MC.13c-2).
...
- Depende de : **lo único que queda antes del asistente v2 es MC.13c-2** (3 decisiones de Esteban, tarjeta abajo); coordinar con PA.1 (conflicto (b)); decisión del conflicto (a) sobre NAV.A2b antes de MC.13e
- Modelo     : análisis/re-corte hecho con Opus 4.8 (ADR 041); MC.13a, MC.13b, MC.13c-1 y MC.13d hechas con Opus 4.8; MC.13c-2 Opus 4.8 - Alto (semántica de pagos), MC.13e+ Sonnet 5 - Alto por rebanada de UI
...
#### MC.13c-2 - La checklist de Necesidades consume el motor (absorbe MC.7g) [3 decisiones de Esteban]
- Prioridad  : alta (es el primer consumidor real del motor)
- Estado     : **bloqueada por decisiones de producto**, análisis hecho el 2026-07-14 al cerrar MC.13c-1. No iniciar sin respuesta a las tres preguntas de abajo.
- Objetivo   : `construirDesgloseNecesidades` (`tesoreria/logic/distribucion.js`) pasa a consumir `obligacionesYAportesDelCobro`, con lo que la checklist deja de mostrar solo fijos Mensuales y pasa a la ventana del cobro (cierra **MC.7g**).
- **Por qué no se hizo junto con MC.13c-1:** el ADR 041 dice que MC.7g se cierra "sin código especial", y el análisis del código lo desmiente. Cambiar la checklist toca tres cosas que no son técnicas:
  1. **Qué significa "ya lo pagaste".** Hoy `_pagadoEstePeriodo` cuenta gastos del **mes calendario**; el motor cuenta lo abonado **dentro de la ventana del cobro**. Con un fijo Quincenal la regla actual está mal (un pago marcaría las dos ocurrencias como pagadas), pero cambiarla altera lo que ve un usuario mensual, que hoy funciona bien.
  2. **Qué registra marcar una fila con varias ocurrencias.** Un fijo Quincenal cubierto por un cobro mensual son 2 vencimientos: `_aplicarNecesidad` (`tesoreria/acciones/distribucion.js`) hoy escribe UN `Gasto` con la fecha de hoy. ¿Un gasto de $120.000 o dos de $60.000? Cambia lo que se ve en Movimientos y en Análisis.
  3. **Reescribir ~20 tests vigentes** de `construirDesgloseNecesidades` que fijan el comportamiento actual (regla 2.5: confirmar antes de tocar tests existentes).
- **Dependencia técnica encontrada:** `ultimoPagoHasta` (`tesoreria/logic/ingresos.js:199`) devuelve `null` para todo lo que no sea Mensual o Quincenal, así que hoy la vista no sabe la fecha del cobro de un usuario Semanal o Diario y no puede construir la ventana. `ventanaDelCobro` cubre todas las frecuencias, pero **datar el último cobro** sigue sin generalizarse: es una rebanada previa (`ocurrenciasEnRango` mirando hacia atrás ya da la pieza).
- Secciones  : Mis cuentas (asistente de distribución)
- Archivos   : `tesoreria/logic/distribucion.js` (`construirDesgloseNecesidades`), `tesoreria/logic/ingresos.js` (`ultimoPagoHasta`), `tesoreria/views/distribucion.js`, `tesoreria/acciones/distribucion.js` (`_aplicarNecesidad`), `tests/unit/tesoreria.test.js`
- Depende de : las 3 decisiones de arriba + generalizar `ultimoPagoHasta`
- Modelo     : Opus 4.8 - Alto (semántica de pagos sobre dinero real, con tests vigentes que se reescriben)
