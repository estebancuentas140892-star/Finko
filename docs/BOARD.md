...
Las 36 tarjetas del tablero, para elegir la próxima sin cargar el archivo completo (principio 9). "Depende de" va acortado a la referencia clave; el texto completo vive en la tarjeta, más abajo por sección.
...
| EDIT.1 | Editar sin destruir: Inversión y Me deben | Transversal | media-alta | nada duro; coordina con ARQ.1 |
...
> **Iniciativa "IN.9 - Inicio en escritorio"** (auditoría de navegación global de Claude Design, 2026-07-31; handoff en `Auditoría navegación global Finko-handoff/`). Fuente única del reparto de Inicio sobre 1024px. Ocho decisiones ID1 a ID8: **ID8 es un no-op** (decide *no* agregar un botón de registrar, ya cubierto por ADR 024) y **ID4 e ID7 salen a INT.1**, porque dependen de una barra superior de escritorio que no existe en el repo. **IN.9c e IN.9d cerraron** (columna propia del detalle por cuenta, D3 del ADR 057; Accesos rápidos en fila propia con Actividad reciente y Resumen semanal en la fila final 6+6, D4 del ADR), queda solo ID6 en IN.9e (ID2 y ID5 ya cerraron). **PI10 quedó resuelto por diseño:** la fila final reparte 6+6 entre Actividad y Resumen, así que una columna de actividad corta ya no desbalancea nada (el gráfico semanal la acompaña con su propio contenido). El informe deja además tres reglas para el design system (R78 ancho completo que se gana, R79 gráfico dimensionado por su dato, R80 acordeón como respuesta al ancho escaso) y dos decisiones abiertas que viajan con INT.1: `--fk-bg-glass` sin valor en tema claro (PI7) y el contraste de los tokens `--fk-dom-*` (PI8, medido: 3,23:1 y 2,65:1, bajo el mínimo 3:1 de WCAG 1.4.11). **Móvil no cambia salvo lo que IN.9a decidió** (orden de las alertas de límites).
...
#### EDIT.1 - Editar sin destruir: Inversión y Me deben
- Prioridad  : media-alta
- Estado     : pendiente, patrón P3 de la auditoría. **Metas (EDIT.1a) y Apartados ya cerraron** y dejaron el patrón validado; quedan Inversión y Me deben, ninguna iniciada.
- Objetivo   : dos secciones todavía no permiten **editar** lo ya creado: corregir un objetivo, una fecha o una tasa obliga a **eliminar y recrear**, perdiendo en el camino el progreso, los aportes y los intereses acumulados. Aplicar el mismo patrón que **EDIT.1a** (Metas) y la rebanada de Apartados ya validaron (formulario reinyectado con `registro = null` para crear y con el registro existente para editar; `normalizarX(datos, existente = null)` conserva el histórico acumulado y recalcula solo lo que depende del campo editado) a Inversión y Me deben, en una rebanada por sección. **Decisión ya tomada, válida para las dos que faltan:** se conserva el histórico tal cual (no se recalcula ni se toca), y el estado derivado (completado/vencida/lo que aplique) se recalcula contra el dato nuevo.
- Secciones  : Inversión, Me deben
- Archivos   : `inversiones/`, `personales/` (form + acciones de cada uno), patrón de referencia en `metas/` (EDIT.1a), `apartados/` y en `compromisos` (D.15b, para Deudas)
...
- Depende de : nada duro. Coordina con **ARQ.1** (si las 4 bolsas comparten componente, el formulario de edición se simplifica): decidir si conviene antes de las 2 rebanadas que faltan, aunque EDIT.1a y Apartados ya demostraron que escribir la rebanada de un dominio no es tan costoso como se temía
- Modelo     : Equilibrado - Alto por rebanada (patrón ya probado en D.15b, EDIT.1a y Apartados; sin lógica financiera nueva salvo la decisión de progreso, ya resuelta)
...
- **Arrancó de lado con DIS.19 (2026-07-29):** dos piezas concretas ya viven en infra, porque el hub de carriles las necesitaba y ADN 10 le prohibía importar esos dominios. **`infra/bolsas.js`** guarda `diasHastaFecha` + `planDeReferencia` + `estadoDeBolsa` (venían de Apartados) y **`infra/portafolio.js`** la cadena de proyección completa, de `esProyectable` a `columnasPortafolio` (venía de Inversión); cada dominio re-exporta con el nombre de siempre, así que no cambió ningún llamador, y `tests/unit/bolsas.test.js` compara identidad de función para que una copia local nueva falle. **Lo que sigue siendo de esta tarjeta:** el modelo unificado (`calcularProgreso` en 3 copias, el `diasHastaFecha` de Metas con otra firma y otro redondeo, los handlers de aportar) y la propiedad "descuenta saldo" del ADR 053. El archivo ya existe: la tarjeta ya no lo crea, lo completa.
...
| Inversión | Sin pendientes propios. Su "editar sin destruir" es una rebanada de **EDIT.1**, pendiente; su infraestructura compartida, **ARQ.1** |
