...
> Última actualización: 2026-07-13 (feat(analisis): ANL.2b patrimonio neto como card-héroe con composición + ojo, ADR 038)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2645/2645 verdes |
| Tests E2E | 199/199 verde. Suites: `smoke` 121 tests, `estrategia-pago` 21 tests, `ahorro-inversion` 9 tests, `hub-ahorros` 7 tests, `navegacion-render` 7 tests, `registrar-destinos` 6 tests, `install-prompt` 6 tests, `a11y-forms` 6 tests, `registrar-sheet` 5 tests, `reflow-320` 4 tests, `registrar-distribucion` 3 tests. |
...
Tercera y última rebanada de Calendario v2 ([ADR 037](DECISIONS/037-calendario-v2-visual.md) D4/D5/D7): **la iniciativa queda completa** (CAL.4a-c en un día, cuarta pantalla de la familia v2 tras Inicio, Mis cuentas y Deudas). El total del detalle cambia de label según el estado real: "Total a pagar" → "Pagado este día" (verde) cuando todos los compromisos del día están completos. Badge de pagado como pill verde con `i-check-circle`; el estado **parcial** ("Abonado $X de $Y este mes") se preserva como pill neutra (mandato del doc de diseño). Los CTA dejan el verde genérico y llevan la identidad del tipo: Marcar pagado índigo agenda, Abonar frambuesa/rosa (criterio `.deuda-card__abonar`, ADR 036: un abono no es un ingreso); Editar/Eliminar intactos. El recordatorio del día de ingreso pasa a callout verde con `i-lightbulb`; Distribuir sigue verde (es ingreso). El ojo del hero enmascara ahora también el total del día, los montos por item y el badge parcial. `.cal-detail__badge-abono`/`.cal-detail__actions` migraron de `domain.css` a `config.css` (junto a su familia). 8 tests unitarios + 1 E2E nuevos. 2634/2634 unit + 197/197 E2E + lint verdes. SW v384→v385.

---

### feat(agenda): CAL.4b grilla legible + selección índigo + empty state del mes · 2026-07-13

Segunda rebanada de Calendario v2 ([ADR 037](DECISIONS/037-calendario-v2-visual.md) D2/D3/D6). `.cal-day` pasa a celda cuadrada centrada (`aspect-ratio: 1`, piso táctil 44px, techo 72px en desktop) con fondo transparente en reposo; la fila de dots se pinta siempre (vacía si no hay eventos) para alinear los números en el grid centrado. El día **seleccionado** cambia de borde neutro a anillo índigo de sección (`--fk-dom-agenda-text` + tinte 16%; variante `-text` por el umbral 3:1 en claro, criterio IV.2c): "hoy" (verde) y "seleccionado" (índigo) ya no se confunden. El subtítulo del mes separa compromisos de ingresos ("2 compromisos este mes · 1 ingreso", ADR 021). Mes sin eventos → card `.cal-empty` con borde punteado, teja índigo y CTA `+ Agregar gasto fijo`; la grilla sigue visible y CAL.3 queda intacta (días vacíos clickeables, el empty state convive con el detalle de un día vacío). 6 tests unitarios + 1 E2E nuevos. 2626/2626 unit + 196/196 E2E + lint verdes. SW v383→v384.
