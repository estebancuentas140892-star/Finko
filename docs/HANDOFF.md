...
> Última actualización: 2026-07-13 (feat(transversal): CAT.2e selector de ícono en Mis cuentas, quinto consumidor)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2589/2589 verdes |
| Tests E2E | 190/190 verde. Suites: `smoke` 112 tests, `estrategia-pago` 21 tests, `ahorro-inversion` 9 tests, `hub-ahorros` 7 tests, `navegacion-render` 7 tests, `registrar-destinos` 6 tests, `install-prompt` 6 tests, `a11y-forms` 6 tests, `registrar-sheet` 5 tests, `reflow-320` 4 tests, `registrar-distribucion` 3 tests. |
...
Segunda rebanada de CAT.2: **Metas** migrada (categoría "Otra"). El `<input type="text" maxlength="4">` para pegar un emoji a mano (MT.3) se reemplaza por `renderIconoPicker`/`wireIconoPicker`; `meta.icono` ahora puede guardar un id de símbolo del sprite (elegido con el picker) o un emoji crudo (metas viejas, backward-compat sin bump de schema: `_iconoMeta()` distingue por el patrón `/^[a-z]-/`, que ningún emoji real produce). El componente compartido gana **`resetIconoPicker(container)`** y un `id` propio en el input oculto: a diferencia de Gastos (que re-renderiza el form completo en cada apertura), el form de Metas es un singleton reusado entre aperturas, así que hay que resetear el estado visual del picker explícitamente al reabrir (`resetModal()` limpia valores pero no el recuadro/panel). **Bug real encontrado y corregido**: el E2E de Gastos (TX.9b) usaba un locator Playwright sin acotar (`page.locator('.icono-picker__panel')`), que se volvió ambiguo en cuanto Metas sumó su propio picker a la página (con 2 dominios migrados, 2 instancias del componente conviven en el DOM desde el arranque, aunque un modal esté cerrado); corregido a `form.locator(...)`. 5 tests unitarios nuevos (`icon-picker.test.js`) + 2 en `metas.test.js`; 2 E2E de `smoke.test.js` reescritos al nuevo flujo + 1 corregido en Gastos. 2548/2548 unit + **183/183 E2E completos** + lint verdes. SW v375→v376.
