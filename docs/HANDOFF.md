...
> Última actualización: 2026-07-12 (feat(ui): MC.17e teja "Transferir" en Registrar, cierra MC.17 completa)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2491/2491 verdes |
...
Última rebanada de MC.17 (transferencias entre cuentas propias): cierra la iniciativa completa (MC.17a fundación de datos, MC.17b formulario/acción, MC.17c ledger de Movimientos, MC.17d GMF del retiro). Nueva teja "Transferir" en la hoja "Registrar" (NAV.A2), visible solo con 2+ cuentas activas (mismo patrón 0/1/varias que ya usan Abono a deuda y Aporte a ahorro en ese mismo archivo). `cuentasActivasParaTransferir(cuentas)` (nueva, pura, en `modules/ui/registrar.js`) cuenta las cuentas activas sin importar el dominio `tesoreria` (regla ADN #10: la hoja lee `S` directamente y reusa acciones ya registradas por nombre). La teja reutiliza la acción `abrir-transferencia` que MC.17b ya registró: cero lógica nueva de apertura de modal. 2 tests unitarios nuevos. 2479/2479 unit verdes. SW v365→v366.
