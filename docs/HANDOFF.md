...
> Revisado: 2026-08-01. Última tarea cerrada: DV.2c, cascada de listas + resaltado de fila + retiro de bucles infinitos.
...
| Tests E2E | 252/253 + 1 flaky (retry verde), corrida del 2026-08-01, sello del commit `680b0f0`. **Es compuerta** desde el 2026-07-30: el hook de pre-commit exige el sello de una corrida verde cuando el diff toca runtime. La huella se calcula sobre el **índice** (`git ls-files -s`), no sobre el árbol: hay que `git add` **antes** de sellar, o el propio `add` invalida el sello. **Ojo con sesiones paralelas sobre el mismo worktree**: el índice compartido puede invalidar el sello entre que se corre la suite y se commitea (visto el 2026-08-01, DV.2b/DV.2c) |
