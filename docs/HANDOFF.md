...
> Última actualización: 2026-07-08 (docs(triaje): lote de 5 auditorías de Esteban integrado al BOARD, regla 2.7)
...
Primer triaje formal bajo 2.7: 5 briefs (Inicio, Gastos, Calendario, Mis Cuentas, Deudas/pagos automáticos) cruzados contra tablero, ADRs, fichas y entre sí. Nuevas: iniciativa **Inicio v2/IN.8** (requiere revisión del ADR 028: reordena la pantalla y reabre la foto de avatar que D3 descartó; absorbe IN.6b, IN.4b y 2 observaciones), iniciativa **CAT** (taxonomía Gastos↔fijos + picker de icono compartido + categorías personalizadas globales; coordinar con ADR 029 D3), **MC.13** (distribución contextual por fecha, absorbe MC.7g, define el motor de vencimientos compartido), **MC.14** (llaves de transferencia), **D.14** (deuda acredita cuenta destino), iniciativa **PA.1** (pagos automáticos, requiere ADR: catch-up al abrir + riesgo de divergencia con la realidad). Integrado a fuentes existentes: specs de color a IV.2, copy de alertas a LIM.1, nota cruzada en ANL.1. Verificado y sin tarjeta: "pagar deuda descuenta cuenta" ya existe (ADR 002). Cruces internos del lote detectados: el picker aparecía en 2 briefs (un componente) y el motor de vencimientos en 2 (una pieza infra). Cero código. Detalle completo en CHANGELOG y [`BOARD.md`](BOARD.md).
