...
> Última actualización: 2026-07-13 (docs(taxonomia): CAT.1 taxonomía global validada con Esteban, ADR 014 aceptado y ADR 029 D3 confirmada)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2598/2598 verdes |
| Tests E2E | 192/192 verde. Suites: `smoke` 114 tests, `estrategia-pago` 21 tests, `ahorro-inversion` 9 tests, `hub-ahorros` 7 tests, `navegacion-render` 7 tests, `registrar-destinos` 6 tests, `install-prompt` 6 tests, `a11y-forms` 6 tests, `registrar-sheet` 5 tests, `reflow-320` 4 tests, `registrar-distribucion` 3 tests. |
...
### feat(transversal): CAT.2f selector de ícono en Gasto fijo/Calendario, sexto consumidor · 2026-07-13

Sexta y última rebanada de CAT.2: **Gasto fijo/Calendario** migrado, categoría "Otro". **Cierra la iniciativa CAT.2 completa.** Análisis previo en la misma sesión (triaje 2.7): pregunta directa a Esteban sobre alcance (mínimo, como CAT.2d/2e, vs. categorías personalizadas completas); decisión: alcance mínimo, sin esperar la validación de taxonomía de CAT.1 (categorías nombradas nuevas quedan para CAT.3). `agenda/view.js`: `renderFormGastoFijo()` agrega el grupo del picker, oculto por defecto; `_renderDetalleItem()` resuelve `c.icono` antes que `CATEGORIA_AGENDA_ICONO[c.categoria]`. `agenda/index.js`: el form se re-renderiza completo en cada apertura (no hace falta `resetIconoPicker`); `_syncCategoriaGastoFijo()` (ya era el listener de `change` de AG.4) se extiende para alternar también la visibilidad del ícono; `setIconoPickerValor` prellena en modo edición. `compromisos/logic/modelo.js`: `normalizarCompromiso()` gana `base.icono` en la rama `tipo==='fijo'` (antes solo existía para deudas, CAT.2d), mismo patrón (categoría "Otro" + valor del catálogo, siempre explícito). `gastos/logic.js`: `iconoPorOrigen()` (TX.6/TX.7, herencia de ícono cuando un gasto nace de pagar un fijo) resuelve `comp.icono` antes que el ícono fijo por categoría. El checklist de "Distribuir mi ingreso" no necesitó ningún cambio: ya generalizaba por `it.icono` sin distinguir tipo desde CAT.2d. 15 tests unitarios nuevos (`compromisos.test.js` + `agenda.test.js` + `gastos.test.js`) + 2 E2E nuevos. 2598/2598 unit + **192/192 E2E completos** + lint verdes. SW v379→v380.
