...
> Última actualización: 2026-07-15 (feat(forms): FORM.1a lenguaje de formularios v2 + Registrar gasto, ADR 042)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2816/2816 verdes |
| Tests E2E | 206/206 verde. Suites: `smoke` 127 tests, `estrategia-pago` 21 tests, `ahorro-inversion` 9 tests, `hub-ahorros` 8 tests, `navegacion-render` 7 tests, `registrar-destinos` 6 tests, `install-prompt` 6 tests, `a11y-forms` 6 tests, `registrar-sheet` 5 tests, `reflow-320` 4 tests, `registrar-distribucion` 3 tests. |
...
Primera rebanada de **Formularios v2** ([ADR 042](DECISIONS/042-formularios-v2-visual.md), octava entrega de la familia visual v2; handoff de Claude Design enviado por Esteban). Nace el lenguaje compartido de captura en `forms.css` (teja de dominio en el header del modal, monto hero tintado, chips de categoría con radios reales, atajos de fecha Hoy/Ayer/Otra fecha, footer con primario a lo ancho) y lo estrena el form flagship, Registrar gasto: el select de categoría desaparece (chips de ícono, catálogo completo + personalizadas + "Otra categoría"), el monto pasa al frente (**revisa TX.9a**, decisión del mockup) y "Hoy" viene elegido por defecto (regla CAT.4 en este form). El contrato FormData y `validarGasto()` no cambian. **Fix preexistente encontrado por la verificación:** la regla móvil anti-zoom de `responsive.css` (16px) le ganaba por orden de capas a `.input--big-amount` y achicaba el monto grande (también en el ingreso puntual); la excepción vive ahora en la misma media query. **Conflicto señalado (ADR 042 D9):** AP.5 pedía dropdown de categoría; la palabra la tiene Esteban al iniciarla. Quedan FORM.1b (Nueva deuda) y FORM.1c (Nuevo gasto fijo). 4 unit nuevos + 3 reescritos; helper E2E `elegirCategoriaGasto` + 9 flujos adaptados. 2816/2816 unit + 206/206 E2E + lint verdes. SW v400→v401.

---

### feat(tesoreria): MC.13c-2 la checklist de Necesidades consume el motor, cierra MC.7g · 2026-07-14

Quinta rebanada de **MC.13** ([ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md) D2/D3): el asistente deja de mostrar "todos los fijos mensuales del mes" y pasa a mostrar **lo que vence antes de tu próximo cobro**, con todas las frecuencias. **Cierra MC.7g**, abierto desde MC.7d. Tres decisiones de producto resueltas con Esteban antes de codificar (regla 2.7): (1) "ya pagado" es por **ventana del cobro**, no por mes calendario (la regla vieja marcaba pagadas las DOS ocurrencias de un quincenal con un solo gasto del mes: era un error real que impedía cerrar MC.7g); (2) marcar una fila registra **un `Gasto` por el total** (2 vencimientos de $60.000 → un gasto de $120.000 con fecha de hoy, no dos con fecha futura); (3) tests adaptados, no borrados. `construirDesgloseNecesidades` delega en `obligacionesYAportesDelCobro` y se borran `_prefijoMes`/`_pagadoEstePeriodo`. **Error de diseño detectado por los tests y corregido**: el primer intento sumaba `vencidas` + `enVentana` y le habría dicho a un usuario mensual que debe dos arriendos; la checklist muestra sólo `enVentana` y lo vencido tendrá su copy en MC.13e, como fijaba el ADR. Generalizar `ultimoPagoHasta` quedó fuera (no hacía falta y toca la clave de de-duplicación): tarjeta MC.13c-3. 24 tests en el bloque (6 nuevos) + el E2E que fijaba "excluye un fijo Quincenal" ahora verifica lo contrario. 2813/2813 unit + 206/206 E2E + lint verdes. SW v399→v400.
