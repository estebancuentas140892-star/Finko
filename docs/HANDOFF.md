...
> Última actualización: 2026-07-15 (feat(tesoreria): MC.13e-2a, copy/accesos cruzados/navegación del asistente "Distribuir mi ingreso", primera de 7 rebanadas de MC.13e-2)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2832/2832 verdes |
...
Primera de las 7 rebanadas de MC.13e-2 (rediseño del asistente "Distribuir mi ingreso", [ADR 041](DECISIONS/041-motor-vencimientos-y-distribucion-v2.md)): 4 puntos de bajo riesgo del brief, cero lógica financiera. **(11)** el array `ctas` ("Ver progreso del fondo", "Ver estrategia de deudas", "Ver tu seguimiento en Límites de gasto"...) deja de renderizarse en la tarjeta compacta; la lógica que los calcula sigue intacta (MC.13e-2g decidirá si vuelven, contextuales a cada paso). **(12)** el aviso "recibiste tu ingreso" pasa a un párrafo propio (`.distribuir-aviso`), separado de la tarjeta, que ahora siempre abre con la misma invitación genérica. **(13)** se retocó el `<legend>` del panel ("hacia" → "entre", "El resto" → "Lo que no distribuyas"). **(17)** los botones Atrás/Siguiente/Distribuir cambian sus flechas de texto plano por íconos del sprite (`i-chevron-right` rotado para "Atrás", `i-check-circle` en "Distribuir"). El E2E que verificaba el CTA cruzado a Límites de gasto se reescribió para verificar su ausencia; 3 tests unitarios nuevos para `renderDistribucionIngreso()`. 2832/2832 unit + 207/207 E2E + lint verdes. SW v405→v406.

---

### docs(triaje): MC.13e-2 análisis y re-corte del rediseño del asistente · 2026-07-15

Solo docs, cero código. El brief de MC.13 (puntos 9-21, rediseño de "Distribuir mi ingreso") venía como un solo bloque ("MC.13e+") demasiado grande para ejecutar de una vez (regla 2.1). Mapeado el código actual (`tesoreria/views/distribucion.js` + `acciones/distribucion.js`) y re-cortado en **7 rebanadas independientes**: **MC.13e-2a** (copy + accesos cruzados + navegación, sin riesgo, primer pick), **MC.13e-2b** (quitar abonos a deuda del asistente), **MC.13e-2c** (logo/ícono + nota por fila), **MC.13e-2d** (cuota del período en vez del objetivo total), **MC.13e-2e** (completar con saldo de otra cuenta si no alcanza, Opus por lógica financiera nueva), **MC.13e-2f** (cuenta del ingreso fijo + decisión explícita del remanente, **bloqueada por una decisión de UX de Esteban**) y **MC.13e-2g** (rediseño en 2 pasos con educación financiera, **bloqueada por decidir si pasa por un handoff de diseño**, como las 8 pantallas v2 anteriores). Orden recomendado: 2a → 2b → 2d → 2c → 2e → 2f → 2g. Detalle completo en `docs/BOARD.md`.
