...
> Última actualización: 2026-07-13 (feat(compromisos): D.15d-2 las 3 palancas a primer plano en la vista, absorbe D.15e)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2520/2520 verdes |
...
Segunda rebanada de D.15d: conecta el motor `recomendarPalanca` (D.15d-1) a la vista. La card de estrategia pasa a **2 niveles ortogonales**: el **orden** (Avalancha/Bola de nieve, arriba, sin cambio) y la **palanca** (Aumentar la cuota / Renegociar / Consolidar), que antes vivía enterrada en el panel "plan inviable" y ahora es una **sección siempre visible** abajo, con intro + la razón de la principal + 3 tiles ordenados por relevancia (`palanca.orden`), la principal marcada "Recomendada" y preseleccionada. La vista calcula la capacidad: `estimarSalarioMensual(S.ingresos)` (infra) para el ingreso + `calcularFijosMensuales(S.compromisos)` (nueva en `modelo.js`, suma tipo 'fijo') para los fijos. **Absorbe D.15e:** el acelerador `<details>` "¿Puedes pagar más rápido?" se retiró; su input de pago extra vive ahora en la palanca "Aumentar la cuota", que ya trae el botón "Aplicar". El panel inviable queda en 2 capas puras (ADR 031): el botón de alerta abre SOLO el diagnóstico (danger); la solución (palancas) es la sección neutra siempre visible. **BUG-011 intacto:** la estructura de la card se sigue decidiendo con datos registrados (extra=0); el detalle de orden se decopló del extra simulado. Bloque unit de la vista reescrito + `describe D.15d-2` nuevo; E2E `estrategia-pago` reescrito (helper `elegirPalanca`, palancas siempre visibles). 2520/2520 unit + **179/179 E2E** + lint verdes. SW v371→v372.

---

### feat(compromisos): D.15d-1 motor puro recomendarPalanca + estimarSalarioMensual a infra · 2026-07-13

Primera de las dos rebanadas en que se re-cortó **D.15d** (motor de recomendación de palanca de Deudas v2), aplicando la regla 2.1 y el precedente **MC.17a** (lógica pura aterriza antes que su consumidor). Dos piezas, sin UI todavía:
**(1) Extracción a infra:** `estimarSalarioMensual` pasa de `tesoreria/logic/ingresos.js` a `infra/financiero.js` (con su tabla de factor de proyección), su hogar único sin dueño de dominio ahora que presupuesto y compromisos también la consumen. El barrel `tesoreria/logic.js` la re-exporta desde infra (consumidores del barrel y tests intactos); `presupuesto/view.js` la importa directo de infra (un import cruzado de dominio menos, ADN 10 más limpio); `tesoreria/acciones/ingresos.js` y `tesoreria/views/distribucion.js` repuntados a infra.
**(2) Motor puro `recomendarPalanca(deudas, { ingresoMensual, fijosMensuales })`** en `compromisos/logic/estrategia.js`: decide la palanca principal (Aumentar/Renegociar/Consolidar) por margen libre real (`ingreso - fijos - Σ cuotas de deuda`) y devuelve el orden de relevancia de las 3 + la razón, en tono ADR 003/008. No lee S ni importa tesorería (recibe la capacidad como parámetro; la vista la calculará en D.15d-2). Umbrales: tasa alta 25% EA (heurística, no la usura del ADR 004) y capacidad mínima 20.000/mes. Nadie consume el motor todavía (eso es D.15d-2). 12 tests unitarios nuevos. 2514/2514 unit + lint verdes. SW v370→v371.
