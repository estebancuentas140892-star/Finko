...
> Última actualización: 2026-07-05 (feat(resumen): IN.4a, accesos rápidos personalizables en Inicio)
...
| Métrica | Valor |
|---|---|
| Tests unitarios + integración | 2188/2188 verdes |
...
Primera fase del [ADR 028](DECISIONS/028-inicio-centro-de-control.md), aprobado el mismo día. "Buenos días / Buenas tardes / Buenas noches, {nombre}" bajo el título de Inicio, usando `S.perfil.nombre` (existía desde el onboarding, ninguna vista lo leía todavía); sin nombre, saluda sin él. Sin dato nuevo ni migración de schema. `updSaludo()` nuevo en `modules/infra/render.js`, mismo patrón que `updSaldo()` (lee `S` directo, se invoca desde `renderAll()`); elemento `#saludo-inicio` en `index.html` reutilizando `.section__subtitle`. 2112/2112 unit (6 tests nuevos con reloj falso). Preview no disponible en este entorno (mismo problema recurrente: el servidor no llega a "running"); verificado por render happy-dom. SW v322 → v323. **Siguiente fase del orden recomendado: CAL.1 (nudge de distribución del ingreso en Inicio).**

| Archivo | Cambio |
|---|---|
| `modules/infra/render.js` | `updSaludo()` nuevo. |
| `index.html` | `#saludo-inicio` bajo el título. |
| `tests/unit/render.test.js` | 6 tests nuevos. |
| `docs/contexto/inicio.md` | IN.6a cerrada. |
| `docs/BOARD.md` | Tarjeta IN.6a borrada. |
