...
> Última actualización: 2026-07-03 (BUG-009 con diseño decidido: cuota + extra con tope coordinado; pendiente de implementación).
...
- Causa     : el checklist (`_leerNecesidadesMarcadas`) y el abono extra (`_leerItemsDistribucion`) leen el saldo ANTES de aplicar nada, así que ambos topan al mismo saldo previo sin descontar lo que el otro va a pagar. Los dos flujos operan sobre la misma deuda sin coordinar su tope. Preexistente al fix de BUG-006 en la matemática de la cuenta (el descuento `descontable` de tesorería ya debitaba `cuota + extra`); el fix solo lo hizo visible al crear el segundo gasto.
- Diseño    : decidido con el usuario (2026-07-03): SÍ se permite pagar cuota + extra a la misma deuda en un mismo movimiento; el tope del extra pasa a ser el saldo restante tras la cuota marcada (`efectivo = min(extra, saldoTotal - cuotaMarcada)`, y si la cuota ya cubre todo el saldo el extra queda en 0 y se ignora). Se descartaron: excluir la deuda de "Abonar extra" cuando su cuota está marcada (elimina un flujo legítimo y exige filas que aparecen/desaparecen en vivo) y bloquear la confirmación con error (fricción, rechaza una intención válida). Implementación: `_leerItemsDistribucion` debe recibir las Necesidades marcadas para restar la cuota del tope (hoy topa contra `_saldoDeuda(id)`, el saldo previo); `_recalcularDistribucion` y `_confirmarDistribucion` ya leen ambas listas, así que el resumen en vivo y el apply comparten el mismo monto efectivo (promesa existente del docstring de `_leerItemsDistribucion`). El cálculo del tope va en un helper puro de logic.js, testeable en unit; el escenario del bug (cuota topada + extra topado sobre la misma deuda) se cubre con E2E.
