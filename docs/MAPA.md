...
| Deudas | `compromisos/` | `logic/` (`modelo.js`, `alertas.js`, `estrategia.js`, `abonos.js`) y `views/` (`alertas.js`, `dashboard.js`, `estrategia.js`, `estrategia-impacto.js`, `formularios.js`, `lista.js`); `logic.js`/`view.js` son barrels e `index.js` el wiring | `charts.css` (chooser entidad/personal, estrategia de pago), `domain.css` (abono-btn, cal-detail) | `compromisos.test.js`, `estrategia-pago.test.js` (e2e) |
