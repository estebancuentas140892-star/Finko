...
- 7 tests unitarios reescritos (`render`, `movimientos`, `accesos`), 1 E2E reescrito y 1 E2E nuevo (la contracara móvil de D10, compuerta contra retirar las dos piezas de las dos plataformas). **Sin bump de SW en esta rebanada**: `service-worker.js` traía el bump v543 → v544 sin commitear de la sesión paralela de MOV.1, y tocarlo se habría llevado su cambio dentro de este commit.
