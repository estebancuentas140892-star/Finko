...
- **Estado actual**     : estable, en evolución. **IN.7**, **IN.6a**, **CAL.1** y **TX.8a** cerradas. Rediseño decidido en [ADR 028](../DECISIONS/028-inicio-centro-de-control.md) (**aprobado el 2026-07-05**): un rol por bloque, orden vertical definido, y fases TX.8b → IN.4a → IN.6b restantes en `BOARD.md`.
- **Verificado contra** : (commit de TX.8a, 2026-07-05).
...
- **Íconos de "Actividad reciente" no heredan el origen exacto del compromiso** (a diferencia de la lista de Gastos, que sí usa `iconoPorOrigen()` de TX.6/TX.7): un abono a deuda o pago de fijo en el panel de Inicio muestra el ícono de su categoría interna (`i-deudas`, `i-recurring`), no el ícono específico del compromiso (ej. la categoría real del fijo en Agenda). Se decidió así para no importar `gastos/logic.js` desde `movimientos/` (ADN 10); revisar si vale la pena mover `iconoPorOrigen()` a `infra/` si la diferencia visual molesta en la práctica.
...
**Cambios pendientes**: fases del [ADR 028](../DECISIONS/028-inicio-centro-de-control.md) en `BOARD.md` (TX.8b, IN.4a, IN.6b); **IN.5** depende de **TX.9** (fuera de esta iniciativa). TX.8b agregará la vista completa de Movimientos (ruta propia) y acotará la lista de Gastos para que deje de mostrar las categorías internas. Metas y apartados siguen sin aparecer en el historial (sin registros fechados por aporte); la categoría interna `'Ahorro'` existe en `CATEGORIAS_GASTO` pero ningún flujo crea gastos con ella todavía (verificado de nuevo al cerrar TX.8a, sigue igual).
