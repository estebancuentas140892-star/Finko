...
> Revisado: 2026-08-20 (ficha 07, ADR 069 D8).
...
- **Estado actual**     : estable. **Ficha 07 (2026-08-20, [ADR 069](../DECISIONS/069-bloque-gastos-en-la-barra-movil.md) D8): la lente deja de estar clavada al mes en curso.** `renderPanelPresupuesto()` leía `new Date()` en cada render, así que retroceder a junio en "Día a día" y saltar acá devolvía al mes actual sin avisar (G4); ahora obedece al reloj del bloque (`infra/mes-bloque.js`). **Es una mejora y es un cambio de comportamiento: poder mirar meses anteriores queda anotado para la ficha 12.** Y cada tope gana la salida a los movimientos que lo forman (G5): emite `movimientos:ver` con su categoría y el rango del mes visible. **G2 queda acotado, no resuelto:** el nombre de la sección promete topes y lo que abre es el plan de tres grupos; al bloque entra la parte que sí es de gastos y la pestaña se llama "Límites", pero **el destino del plan de tres grupos lo decide la ficha 12**, porque uno de sus grupos es Ahorro (el otro bloque de la barra) y su dependencia es Mis cuentas. **DSK.8 (2026-08-18, [ADR 077](../DECISIONS/077-limites-de-gasto-tres-grupos-a-la-vista.md)) le da composición de escritorio sin romper la contención del [ADR 019](../DECISIONS/019-limites-por-rol.md)**: desde 1440px Necesidades y Ahorro van en una columna de `span 5` (dentro del envoltorio `.grupos-resumen__col`) y Estilo de vida en una de `span 7`, con sus sobres y sus huérfanas a dos columnas y sus dos párrafos a 62ch. Los topes siguen dentro de su grupo. En **todos los anchos**: el sobre excedido recupera AA (subtítulo y nota a `--fk-text-secondary`, 5,83:1 sobre el fondo compuesto del estado) y el botón de crear un tope se llama "Nuevo límite" en sus dos apariciones, con uno solo visible a la vez. **Iniciativa LIM.1 cerrada** con sus tres rebanadas (LIM.1a el dinero extraordinario, LIM.1b Streaming y Suscripciones contra Estilo de vida, LIM.1c las sugerencias). Los ADR 044 y 045 quedaron Aceptados. La sección ya no solo controla: **propone dónde y cuánto poner tope**, y avisa de la suscripción que lleva medio año cobrándose. DIS.7 (2026-07-26) sigue siendo la base visual de la sección.
- **Verificado contra** : ficha 07 de la auditoría móvil (2026-08-20, ADR 069 D8). Antes: ficha 06 (2026-08-20, ADR 080 D4, las dos salidas hacia el reparto). Antes: `34dc9d5` (2026-08-13, LIM.1c); el resto de la sección, contra `8d4a5be` (2026-07-26, DIS.7).
...
- `2026-08-20 ficha 07 (ADR 069 D8)`: la lente obedece al reloj del bloque en vez de a `new Date()`, y cada tope ofrece "Ver movimientos" (G5). Las cuatro filas de "Se controla en..." pasan a nombrar "Por pagar".
