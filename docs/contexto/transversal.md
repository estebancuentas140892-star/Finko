...
- 2026-07-13 (CAT.2f): Gasto fijo/Calendario migrado como sexto y último consumidor, categoría "Otro". Cierra la iniciativa CAT.2 completa. Análisis previo (misma sesión): pregunta directa a Esteban sobre alcance (mínimo vs. categorías personalizadas completas), decisión: alcance mínimo, sin esperar la validación de taxonomía de CAT.1. `agenda/view.js`: `renderFormGastoFijo()` agrega el grupo del picker, oculto por defecto; `_renderDetalleItem()` resuelve `c.icono` antes que `CATEGORIA_AGENDA_ICONO[c.categoria]`. `agenda/index.js`: el form se re-renderiza completo en cada apertura (`_inyectarFormGastoFijo()`, no hace falta `resetIconoPicker`); `_syncCategoriaGastoFijo()` (ya era el listener de `change` de AG.4) extendido para alternar también la visibilidad del grupo del ícono; prefill con `setIconoPickerValor` en modo edición. `compromisos/logic/modelo.js`: `normalizarCompromiso()` gana `base.icono` en la rama `tipo==='fijo'` (antes solo existía en la rama de deuda, CAT.2d), mismo patrón (`categoria==='Otro'` + valor en el catálogo, siempre explícito). `gastos/logic.js`: `iconoPorOrigen()` (TX.6/TX.7, herencia de ícono cuando un gasto nace de pagar un fijo) resuelve `comp.icono` antes que `CATEGORIA_AGENDA_ICONO[comp.categoria]`. El checklist de "Distribuir mi ingreso" no necesitó ningún cambio: ya generalizaba por `it.icono` sin distinguir tipo desde CAT.2d. 15 tests unitarios nuevos (`compromisos.test.js` + `agenda.test.js` + `gastos.test.js`) + 2 E2E nuevos. 2598/2598 unit + 192/192 E2E completos + lint verdes. SW v379→v380.

- 2026-07-13 (CAT.2e): Mis cuentas migrada como quinto consumidor, banco "Otro". `cuenta.icono` YA EXISTÍA en el schema (sin uso real, ver Riesgos): esta rebanada lo redefine (id de sprite del catálogo compartido, solo cuando `banco==='Otro'`, siempre explícito `null`/id válido) y retira `_iconoPorBanco()` (el emoji que rellenaba el campo para TODOS los bancos sin que nada lo leyera). `infra/bancos.js`: `bancoAvatar(bancoId, icono)` gana el segundo parámetro, aplicado solo si `bancoId==='Otro'` y `icono` tiene forma de id de sprite (`/^[a-z]-[a-z0-9-]+$/`, protege contra el emoji legado). Los 6 call sites de `bancoAvatar` (`cuenta-helper.js` ×3, `render.js`, `tesoreria/views/transferencias.js`, `tesoreria/views/cuentas.js`) pasan `cuenta.icono`. `tesoreria/views/cuentas.js`: `renderFormCuenta()` agrega el grupo `#form-group-icono` con el picker, oculto por defecto. `tesoreria/acciones/cuentas.js`: el form de cuenta es un singleton reusado (como Metas): `wireIconoPicker` una sola vez en `inyectarFormCuenta()`, `_toggleCamposPorClase()` alterna la visibilidad según la clase del banco elegido (`clase==='otro'`), `_resetBankPicker()` llama `resetIconoPicker` y `_editarCuenta()` usa `setIconoPickerValor` para prellenar el ícono guardado. 12 tests unitarios nuevos (`tesoreria.test.js` + `bancos.test.js`, incluye el guard contra emoji legado) + 2 E2E nuevos. 2589/2589 unit + 190/190 E2E completos + lint verdes. SW v378→v379.
