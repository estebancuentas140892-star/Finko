...
| Catálogo de íconos para categoría personalizada (compartido; 29 íconos, reusado por Gastos, Metas y Apartados) | `modules/core/constants.js` | `ICONOS_CATEGORIA_PERSONALIZADA` | ~493 |
...
- **Múltiples instancias del componente conviven en la página** (hallazgo de CAT.2b, sigue vigente con 3 dominios migrados): cada dominio inyecta su modal al arrancar la app, estén o no abiertos, así que hay hasta 3 nodos `.icono-picker__panel` en el DOM a la vez. Un selector Playwright/CSS **sin acotar al formulario** (ej. `page.locator('.icono-picker__panel')`) es AMBIGUO y falla; hay que acotar siempre al contenedor del formulario o al `[data-icono-picker="<id>"]` específico. Los E2E de Gastos, Metas y Apartados ya siguen este criterio.
...
- **`apartado.icono`/`meta.icono` con dos formatos posibles, sin bump de schema**: id de símbolo del sprite (`c-pesa`, elegido con el picker) o emoji crudo (plantillas de Apartados, o datos viejos anteriores a CAT.2b/c). Ambos dominios distinguen por el patrón `/^[a-z]-/` (`_iconoApartado()`, `_iconoMeta()`; ningún emoji real lo produce). Si en el futuro se agrega un catálogo de íconos con otro prefijo de letra, revisar que esta regex lo siga cubriendo en los 2 sitios.
