...
| Catálogo de íconos para categoría personalizada (compartido; 29 íconos, reusado por Gastos, Metas, Apartados y Deudas) | `modules/core/constants.js` | `ICONOS_CATEGORIA_PERSONALIZADA` | ~493 |
...
**Dependencias y relaciones**: `infra/icon-picker.js` no importa de ningún dominio ni lee `S` (recibe el catálogo de íconos y el valor actual por parámetro, mismo patrón que `cuenta-helper.js`/`renderSelectorCuenta`). Cada dominio consumidor pasa su propio catálogo de íconos (Gastos, Metas, Apartados y Deudas reusan `ICONOS_CATEGORIA_PERSONALIZADA` para la grilla; futuros consumidores podrían pasar catálogos propios, a decidir por rebanada).
...
- **`wireIconoPicker` debe llamarse en cada apertura del formulario, O una sola vez si el form es un singleton reusado.** Gastos y Apartados re-renderizan el HTML completo en cada apertura (`_montarFormGasto`/`_inyectarFormApartado`), así que re-adjuntan los listeners cada vez y NO necesitan `resetIconoPicker` al reabrir (el HTML nuevo ya nace limpio). Metas inyecta el form UNA sola vez al arrancar (`_inyectarForm`, en `initMetas()`) y lo reutiliza: ahí sí hace falta `resetIconoPicker()` explícito al reabrir, porque `resetModal()` limpia el `.value` de los inputs pero no el estado VISUAL del picker.
- **Múltiples instancias del componente conviven en la página** (hallazgo de CAT.2b, sigue vigente con 4 dominios migrados): cada dominio inyecta su modal al arrancar la app, estén o no abiertos, así que hay hasta 4 nodos `.icono-picker__panel` en el DOM a la vez. Un selector Playwright/CSS **sin acotar al formulario** (ej. `page.locator('.icono-picker__panel')`) es AMBIGUO y falla; hay que acotar siempre al contenedor del formulario o al `[data-icono-picker="<id>"]` específico. Los E2E de Gastos, Metas, Apartados y Deudas ya siguen este criterio.
...
- **CAT.2e (Cuentas)**: el banco "Otro" en `BANCOS_CO` cae al fallback de iniciales "?" (`tejaMarca`), sin ícono elegible. Igual que Deudas antes de CAT.2d, agrega una capacidad nueva, no solo cambia la UI.
...
- 2026-07-13 (CAT.2d): Deudas migrada como cuarto consumidor, categoría "Otra" (entidad) / "Otro" (personal). A diferencia de los 3 consumidores previos, aquí no había ningún campo de ícono que reemplazar: se AGREGA `compromiso.icono` nuevo (campo opcional sin bump de schema, siempre explícito `null`/id de sprite, ver Riesgos). `formularios.js`: `renderFormDeuda()` agrega el grupo `#grupo-comp-icono` con `renderIconoPicker(ICONOS_CATEGORIA_PERSONALIZADA, { id: 'comp-icono' })`, oculto salvo que la categoría (guardada, en edición) ya sea la de "otra". `index.js`: `_wireIconoOtraCategoria()` nueva alterna `hidden` del grupo al cambiar el `<select>` de categoría y llama `wireIconoPicker` (form re-renderizado completo en cada apertura, como Gastos/Apartados: no hace falta `resetIconoPicker`). `logic/modelo.js`: `normalizarCompromiso()` guarda `base.icono` solo si la categoría normalizada es la de "otra" Y el valor está en el catálogo (protege contra manipulación del DOM), siempre explícito para sobrevivir el merge shallow de `editar()`. `lista.js`: `compromiso.icono` antes que `_ICONO_DEUDA[categoria]` en la teja y el chip. Consistencia de paso: `tesoreria/logic/distribucion.js` (`construirDesgloseNecesidades`) y `views/distribucion.js` (`_iconoNecesidad`) propagan el mismo campo para que el checklist de "Distribuir mi ingreso" muestre el mismo ícono que la lista de Deudas. 14 tests unitarios nuevos en `compromisos.test.js` + 2 en `tesoreria.test.js` + 2 E2E nuevos. 2577/2577 unit + 188/188 E2E completos + lint verdes. SW v377→v378.

- 2026-07-13 (CAT.2c): Apartados migrado como tercer consumidor (nombre del apartado). `infra/icon-picker.js` gana **`setIconoPickerValor(container, valor, previewHtml)`** (fija un valor externo al catálogo, sin pasar por la grilla; ningún botón queda marcado si el valor no coincide con ninguno) y `renderIconoPicker` ahora acepta `label: ''` para omitir el `<span>` de etiqueta en usos compactos (el `aria-label` del panel cae a "Elegir ícono" cuando el label está vacío, para no perder accesibilidad). `apartados/view.js`: el `<input type="text" maxlength="4" placeholder="📦">` se reemplaza por `renderIconoPicker(ICONOS_CATEGORIA_PERSONALIZADA, { id: 'apartado-icono', label: '' })` dentro de la misma columna angosta (`.apartado-nombre-row`, 3.5rem) que antes ocupaba el input de emoji; `_iconoApartado()` nueva distingue sprite-id de emoji crudo (mismo patrón `/^[a-z]-/` que `_iconoMeta()`). `apartados/index.js`: `wireIconoPicker` en `_inyectarFormApartado()` (el form se re-renderiza completo en cada apertura, como Gastos, no como Metas: no hace falta `resetIconoPicker`); `_aplicarPlantilla()` usa `setIconoPickerValor` para las 17 plantillas rápidas (SOAT, Regalos...), que conservan su propio catálogo curado de emojis en vez de migrar al genérico del picker. CSS nuevo en `domain.css`: el panel se vuelve popover flotante (`position: absolute`) cuando vive dentro de `.apartado-nombre-row`, para no romper el layout de 2 columnas al desplegarse. E2E de Apartados: **primera cobertura E2E de la sección** (no tenía ninguna), 3 tests nuevos (plantilla, ícono manual del picker, plantilla + cambio manual). 9 tests unitarios nuevos (`icon-picker.test.js`: label condicional + `setIconoPickerValor`) + 6 en `apartados.test.js`. 2563/2563 unit + 186/186 E2E completos + lint verdes. SW v376→v377.
