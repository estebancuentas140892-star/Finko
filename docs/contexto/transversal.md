...
**Dos puntos NO se tocan** (decisión de Esteban, 2026-08-02, mismo criterio que ARQ.2 punto 3): **(1) handlers de "aportar"**, `_guardarAbonoMeta` (`metas/index.js`) y `_guardarAporte` (`apartados/index.js`) siguen casi idénticos línea por línea. El cálculo que hacían distinto ya está unificado desde ARQ.1a; lo que queda es orquestación de UI (`confirmar()`, DOM, `announce()`), no lógica de negocio, y `infra/bolsas.js` se declara "sin DOM" a propósito. `_ajustarSaldoCuenta`, la pieza más chica, ya se repite igual en 7 dominios de toda la app: unificarla solo acá sería una excepción arbitraria. **(2) etapa de Inversión en el carril de Ahorro** (DIS.18): `_estadoInversion()` (`ahorro/logic.js`) sigue mostrando un conteo ("2 inversiones") en vez de la etapa de `momentoInversion()` (`inversiones/logic.js`). Exponerla vía `infra/portafolio.js` es un cambio de UI real, no un refactor mecánico de cálculo duplicado: queda fuera del alcance de "infraestructura compartida sin lógica de producto nueva" que define esta tarjeta. Ambos quedan como duplicación/hueco intencional, sin tarjeta propia, hasta que alguien decida resolverlos.
