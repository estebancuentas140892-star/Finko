...
- **Estado actual**     : **ARQ.1a cerrada** (2026-08-02, commit `87b6b04`): `progresoDeBolsa(objetivo, actual)` en `infra/bolsas.js` reemplaza las tres copias de `calcularProgreso`/`calcularProgresoFondo` (Metas, Apartados, Ahorro) y la cuarta inline de `estadoDeBolsa`, con el criterio de bordes más estricto (el del fondo). `diasHastaFecha` de Metas (la copia con otro redondeo que arriesgaba el refactor) se retiró sin reemplazo: no la llamaba nadie en `modules/`. Cada dominio conserva su envoltorio y su vocabulario (`completado` vs. `completada`). Es la pieza que resuelve **DOC.1** (Hub Ahorro necesita leer el progreso de las cuatro bolsas sin importar sus dominios, ADN 10): la fuente única ya vive en `infra/bolsas.js`, junto a `estadoDeBolsa` y `planDeReferencia` (arrancados por DIS.19). **ARQ.1b cerrada** (2026-08-02, commit `bc25fe9`): `descuentaSaldo(tipoBolsa, registro)` en el mismo archivo expone en código la tabla del ADR 053 I2 (Metas y Apartados siempre descuentan, el Fondo nunca por ADR 020, Inversión según `cuentaId` de INV.1), con 4 tests de invariante en `analisis.test.js`. `calcularActivos()` no llama esta función a propósito: su regla de suma no se toca de forma retroactiva (ADR 053 I4), y uno de los tests documenta esa brecha aceptada (inversión sin `cuentaId` igual suma a activos). **Pendiente de ARQ.1 (sin tarjeta propia todavía)**: los handlers de "aportar" (casi idénticos entre Metas y Apartados) y la etapa de Inversión (`momentoInversion()`) para el carril de la casa (DIS.18). **ARQ.2 cerrada el 2026-08-02** (puntos 1 y 2; punto 3 analizado y dejado sin tocar, ver abajo). Tarjeta **ARQ.1** sigue en `docs/BOARD.md`, recortada a lo que falta.

**ARQ.1 - un solo modelo para las cuatro bolsas** (`infra/bolsas.js`): fondo de emergencia, metas, apartados e inversión son 4 implementaciones del mismo concepto (bolsa con objetivo, acumulado, progreso y aportes). **ARQ.1a (progreso) y ARQ.1b (descuenta saldo) cerradas, ver arriba.** Queda: los handlers de "aportar" y la etapa de Inversión que la casa de Ahorro necesita mostrar sin importar `inversiones/`. Precedente exacto `infra/vencimientos.js` (MC.13a/b). **No fusiona las pantallas**: las 4 secciones responden a mentalidades distintas del usuario, se unifica la infraestructura, no la UX. Archivos: `modules/infra/bolsas.js`, `metas/logic.js`, `apartados/logic.js`, `ahorro/logic.js`, `inversiones/logic.js`.
