...
- **No se puede editar una inversión** (familia de **EDIT.1**): corregir una tasa mal escrita obliga a eliminarla y volver a crearla, perdiendo la fecha de inicio. Es el hallazgo más caro de la sección y no lo resuelve DIS.17. Desde INV.1 esto pesa más: eliminar y volver a crear con cuenta de origen implica un descuento y una devolución de más, aunque el saldo final cuadre.
...
**Cambios pendientes**: **el momento 3** y el dato que lo habilita (valor real en el tiempo), que pasa por triaje antes de ser tarjeta. **Editar una inversión** (rebanada de **EDIT.1**): el mockup pone Editar al lado de Eliminar en la tarjeta de cada holding y no se implementó porque el flujo no existe. El **consolidado del hub** (hallazgo A13) ya no vive acá: DIS.18 lo mudó a la casa de Ahorro y esta sección abre con `.section__volver` ([ADR 056](../DECISIONS/056-la-casa-de-ahorro.md)).
