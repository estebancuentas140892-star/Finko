...
- **El corte del momento ya no vive acá, las frases sí (ARQ.1c).** `momentoInversion()` pide el número a `etapaDePortafolio()` (`infra/portafolio.js`) porque la casa de Ahorro necesita la misma etapa para su carril y ADN 10 le prohíbe importar esta sección. Si el umbral cambia (hoy: una inversión con monto es el momento 1, dos o más el 2), se cambia allá y las dos pantallas se mueven juntas. Lo que sigue siendo de la sección son los títulos, la explicación del tipo, el anticipo y la acción: infra no guarda copy, cada pantalla pone su vocabulario. `TOTAL_MOMENTOS` tampoco bajó: el conteo "momento 1 de 3" es del relato de esta sección y no lo lee nadie más.
