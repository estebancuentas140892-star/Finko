...
**Cambios pendientes**: de la iniciativa "Deudas v2" quedan las rebanadas funcionales (ver `docs/BOARD.md`): **D.15a** (solo copy de simulaciones + Abonar; su capa visual la cerró D.16c), **D.15b** (editar deuda + reorden del form; el trigger vivirá en la `.deuda-card` nueva), **D.15d** (motor de recomendación de palanca, la siguiente a implementar) y **D.15e** (botón "Aplicar" en el acelerador del plan viable). **D.15c quedó absorbida y cerrada por D.16d.**
