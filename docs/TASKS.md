...
**App estable, 1752/1752 unit + 76/76 E2E verdes, lint limpio.** Último cambio: **ADR 019** (diseño de MC.8): Límites de gasto con tratamiento asimétrico por rol (monitorear/celebrar/controlar), revisa el ADR 017. Solo docs, sin cambio de app (SW sigue v244). Siguiente: **MC.8a** (reencuadre de mensajes por rol). **MC.7 en pausa** (íbamos por MC.7d), se retoma tras MC.8. Antes: MC.7a-c (asistente Distribuir mi ingreso). **Épica EP completa. Rediseño visual 2026 completo: las 8 fases cerradas.**
...
✅ **Renombre de secciones principales: Dashboard → Inicio, Agenda → Calendario.** Evaluación UX/IA (el usuario pidió elegir la mejor opción para un usuario primerizo, aunque difiriera de su sugerencia): se coincide con su propuesta. "Dashboard" es jerga opaca para el público objetivo; "Inicio" es el término convencional y no limita la sección a una función (descartadas "Hoy", que subdescribe, y "Agenda", que colisionaría). "Agenda" → "Calendario" porque para un usuario primerizo gana el reconocimiento del formato (la sección es visualmente un calendario mensual); **reconsidera AG.1**, que antes se inclinaba por "Agenda". Implementado con la guía de IA: **solo se cambió la etiqueta visible**, la ruta (`#dash`, `#agenda`), los `id` del DOM y el nombre del dominio en el código (`agenda`) quedaron estables. Cambios: labels de nav (sidebar + bottom nav) y sus `aria-label`, título de sección "Mi Agenda" → "Calendario", banner de propósito, y 7 menciones de copy en Gastos/Tesorería/Deudas/Ahorro que decían "...desde Agenda" o "...en Agenda". Bonus fix adyacente: la copia de recomendación de distribución (`tesoreria/logic.js`) decía "tus deudas en Compromisos", nombre ya obsoleto de la sección Deudas; corregido de paso. 1 assert E2E actualizado (`smoke.test.js`, heading "Calendario"). 1667/1667 unit + 65/65 E2E verdes. SW v235 → v236 - 2026-06-30. Ver [CHANGELOG](CHANGELOG.md).
...
✅ **MC.7 (diseño) = [ADR 018](DECISIONS/018-asistente-distribuir-ingreso.md).** "Distribuir mi ingreso" (MC.4a-e) evoluciona a un asistente guiado de 3 pasos, donde el usuario solo revisa, ajusta y confirma. Decisiones cerradas con el usuario: (1) **Paso 1 Necesidades** = preview read-only (itemiza gastos fijos de Agenda + cuotas de deuda + compromisos, sin mover dinero ni schema); (2) **Paso 2 Ahorro** = aportes auto-calculados por objetivo (`faltante / periodos restantes` con fecha; 0 + hint sin fecha; fondo con el excedente); (3) **Paso 3 Estilo de vida** = reparto entre cuentas, omitido con cuenta única; (4) objetivos sin fecha = sugerir 0, no adivinar; (5) arrancar por el Paso 2. Confirmación única, reusa apply-plan/undo + gating por fecha de MC.4. Sin schema nuevo en v1 - 2026-07-01. Ver [CHANGELOG](CHANGELOG.md).

Slices de implementación de MC.7 (smallest-first, ver ADR 018). **⏸ En pausa desde 2026-07-01 (tras MC.7c): se prioriza cerrar MC.8 (rediseño de Límites por rol, ADR 019) porque comparten el modelo de distribución. Retomar en MC.7d.**
...
✅ **MC.7c** - Paso 1: `construirDesgloseNecesidades()` en `tesoreria/logic.js`: **desglose itemizado de Necesidades** (gastos fijos + deudas activos, con nombre/categoría/monto mensual), ordenado de mayor a menor. Preview de solo lectura: no mueve dinero. En la vista, `<details>` colapsable bajo la fila de Necesidades del panel, reusando el patrón `.analisis-grupo` con clases propias. 11 unit + 1 E2E nuevos. 1741/1741 → 1752/1752 unit; 75/75 → 76/76 E2E. Verificado en el navegador. SW v243 → v244 - 2026-07-01. Ver [CHANGELOG](CHANGELOG.md).
...
- **MC.7f (opcional)** - Pulido: copy por paso, transiciones, a11y del asistente (foco al avanzar, `aria` de pasos), estados vacíos. Modelo: Sonnet 4.6 - Bajo.

✅ **MC.8 (diseño) = [ADR 019](DECISIONS/019-limites-por-rol.md).** Límites de gasto con **tratamiento asimétrico por rol**, revisa las decisiones 1, 4 y 5 del ADR 017 sin revertir su núcleo. Decisiones cerradas con el usuario: (1) cada grupo según su rol: **Necesidades** = monitorear (sin límites, copy informativo "usan el X% de tu ingreso", no "te pasaste"), **Ahorro** = celebrar (refuerzo cálido al cumplir/superar, nunca alerta), **Estilo de vida** = controlar (único con topes); (2) los topes se **fusionan dentro de la tarjeta de Estilo de vida** (adiós al bloque suelto), con "agregar bajo demanda" + conciencia de "olla finita", rechazando el 100% obligatorio; (3) layout desktop = Necesidades + Ahorro en 2 columnas, Estilo de vida en fila completa; (4) todas las categorías siguen limitables (reclasificar por grupo se difiere). Sin schema nuevo - 2026-07-01. Ver [CHANGELOG](CHANGELOG.md).

Slices de implementación de MC.8 (smallest-first, ver ADR 019):
- **MC.8a** - `presupuesto/logic.js`: reencuadre de `generarMensajesLimites` por rol. Necesidades pasa de alerta con "límite" a mensaje informativo (% del ingreso, sin alarma); Ahorro enriquece el refuerzo; Estilo de vida se conserva. Helper puro para la "olla finita" si no existe ya. Tests. Modelo: Sonnet 4.6 - Medio.
...
✅ **MC.5c** - Desglose por item dentro de cada grupo: detalle colapsable ("Ver detalle") en las tarjetas de Necesidades (`desgloseNecesidadesDelMes`: por fijo/deuda, con monto de referencia y estado pendiente/parcial/pagado del mes) y Ahorro (`desgloseAhorroDelMes`: por destino, fondo/metas/apartados/inversiones; solo el fondo reporta "este mes", el resto acumulado a la fecha). Estilo de vida no lo necesita (su detalle ya es la sección de topes por categoría de MC.5b). Ambas funciones puras, sin importar otros dominios. 16 unit + 2 E2E nuevos. 1691/1691 → 1707/1707 unit; 67/67 → 69/69 E2E. SW v238 → v239 - 2026-06-30. Ver [CHANGELOG](CHANGELOG.md).
✅ **MC.5d** - Alertas y refuerzos inteligentes por grupo y por item: `generarMensajesLimites()` en `presupuesto/logic.js`, reusando el sistema de nudges ya existente (`.nudge nudge-medium|nudge-high|nudge-success`). Alerta por categoría de Estilo de vida (75-99%/excedida, reusa `alertasLimites`), alerta de grupo para Necesidades/Estilo de vida, refuerzo de Ahorro cuando `pct >= 100` (ahí superar el 100% es bueno), y refuerzo combinado cuando todas las Necesidades del mes están pagadas y el grupo no está excedido. Función pura, sin importar otros dominios. 19 unit + 2 E2E nuevos. 1707/1707 → 1726/1726 unit; 69/69 → 71/71 E2E. SW v239 → v240 - 2026-06-30. Ver [CHANGELOG](CHANGELOG.md).
✅ **MC.5e** - CTAs cruzados con Mis cuentas + copy de complementariedad + alineación del banner de propósito (EP.2, ADR 016): `sugerirDistribucionIngreso` agrega un CTA incondicional "Ver tu seguimiento en Límites de gasto"; la nota de Límites y el banner de propósito nombran explícitamente "Mis cuentas planifica, Límites vigila" (ADR 017 decisión 7). 2 unit + 2 E2E nuevos. 1726/1726 → 1728/1728 unit; 71/71 → 73/73 E2E. SW v240 → v241 - 2026-06-30. Ver [CHANGELOG](CHANGELOG.md). **MC.5 completa (a-e).**

Nota: si más adelante se resuelven **MC.10/MC.11** (piso de ahorro + detección de déficit en el modelo de distribución), el asignado por grupo de Límites mejora automáticamente sin tocar este código (Límites consume `sugerirDistribucionIngreso` directamente).
...
✅ **AG.1 = Agenda → Calendario.** Reconsiderado con el usuario: se decide **Calendario** (reversa del lean inicial hacia "Agenda"), por reconocimiento inmediato del formato para un usuario primerizo. Implementado junto con **Dashboard → Inicio** - 2026-06-30. Ver entrada completa arriba y [CHANGELOG](CHANGELOG.md).
...
**Identidad visual de categorías en Gastos (2026-06-30).** Extienden la visión de consistencia visual de ADR 014 ("mismo ícono en Agenda, Gastos, Límites, Dashboard"). Raíz común: un gasto originado en un fijo o una deuda lleva `compromisoId` que enlaza a su fuente ([gastos/index.js:78](../modules/dominio/gastos/index.js)), pero en la lista de Gastos aparece con ícono genérico. El item de Gastos ([`_renderGastoItem` en gastos/view.js:201](../modules/dominio/gastos/view.js)) usa `CATEGORIA_EMOJI[catKey] ?? icon('gastos')`; el fallback genérico salta cuando la categoría del gasto no está en `CATEGORIA_EMOJI` (mapa de Gastos, distinto de `CATEGORIA_AGENDA_EMOJI`).

- **TX.6 - Gastos de fijos heredan el emoji de su categoría de Agenda.** Cuando un gasto fijo se refleja en Gastos, mostrar el emoji de su categoría (🏠 Arriendo, 💡 Servicios públicos, 🛒 Mercado, 📶 Plan de datos, 🏋️ Gimnasio, 📺 Suscripciones) en vez del ícono genérico. Resolver vía `gasto.compromisoId` la categoría del fijo y hacer que el lookup de emoji caiga a `CATEGORIA_AGENDA_EMOJI` (o a un lookup unificado) para gastos originados en Agenda. Respeta TX.4 (etiquetas compartidas = mismo emoji). Modelo: Sonnet 4.6 - Medio.
- **TX.7 - Gastos de deudas muestran 🏦 vs 🤝 según el tipo.** Los abonos a deuda crean un gasto con `categoria: 'Deudas'` + `compromisoId` ([compromisos/index.js:295](../modules/dominio/compromisos/index.js)), así que todas las deudas se ven igual en Gastos. Diferenciar: resolver el `compromisoId` y mostrar 🏦 para `deuda-entidad` y 🤝 para `deuda-personal`, coherente con la identidad de la sección Deudas. Comparte el mismo hook que TX.6 (resolver `compromisoId` en el render del item), así que conviene hacerlas juntas. Modelo: Sonnet 4.6 - Medio.

> Nota: TX.6, TX.7 y AG.2 son la misma idea del usuario (misma categoría ⇒ mismo ícono en toda la app) aplicada a Gastos y Agenda. Conviene un solo pase que unifique el lookup de emoji por origen y lo aplique en los tres renders.
