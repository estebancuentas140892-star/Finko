...
**App estable, 1728/1728 unit + 73/73 E2E verdes, lint limpio.** Último cambio: **ADR 018** (diseño de MC.7): "Distribuir mi ingreso" evoluciona a un asistente guiado de 3 pasos. Solo docs, sin cambio de app (SW sigue v241). Siguiente: **MC.7a** (primer slice, auto-cálculo de aportes de ahorro por objetivo). Antes: **MC.5 completa (a-e)**, Límites de gasto como centro de control. **Épica EP completa. Rediseño visual 2026 completo: las 8 fases cerradas.**
...
- **MC.7a** - `logic.js` puro: función que devuelve el **desglose de aportes de ahorro por objetivo** (una fila por meta/apartado con su aporte sugerido: `faltante / periodos restantes` con fecha, 0 sin fecha; fondo con el excedente si está incompleto). Reusa la fórmula de `calcularAporteMensualObjetivos`. Tests unitarios. Modelo: Sonnet 4.6 - Medio.
