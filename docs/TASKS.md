...
**App estable, 1633/1633 tests verdes, lint limpio, 61/61 E2E.** Último cambio: **D.7** (diseño) revisión D.7 de ADR 011: el bloque inviable pasa a botón único → panel con selector. Antes: D.6 quitar aviso de fijos vencidos de Deudas. **Rediseño visual 2026 completo: las 8 fases cerradas.**
...
**Siguiente sugerido: D.8** (jornada 2 de "Visión de Deudas"): implementar el panel de alternativas con selector que la Revisión D.7 de ADR 011 dejó diseñado. Botón único → panel → selector de una alternativa a la vez, reubicando renegociar y consolidar. Sin lógica financiera nueva. Modelo: Sonnet 4.6 - Alto. Después: D.9 (Aumentar la cuota aplicable, Opus 4.8 - Alto) y D.5 (categorías de deuda en dos dimensiones, Opus 4.8 - Alto).
...
- **D.8 (implementa D.7) - Panel de alternativas con selector** - Botón único de alerta que alterna `panelAlternativasAbierto`; panel con diagnóstico + selector de 3 alternativas (`alternativaActiva`, default "Aumentar la cuota") que muestra **solo el contenido de la opción elegida** (no las tres a la vez). Renegociar (D.3a) y Consolidar (D.3b) ya existen como herramientas Simular + Aplicar: aquí se **reubican** dentro del selector sin tocar su lógica. "Aumentar la cuota" sigue siendo simulación hasta que D.9 le sume Aplicar. Estado del panel en `_uiEstrategia`, 2 data-actions nuevos (`abrir-panel-alternativas`, `elegir-alternativa`). El diagnóstico baja a dentro del panel; el bloque inviable se mueve **debajo del detalle** (corrige la deriva de que hoy va arriba del picker). Verificar el cableado por E2E (preview envenenado). Modelo: Sonnet 4.6 - Alto (reorganización de UI con estado de selector, sin lógica financiera nueva).
