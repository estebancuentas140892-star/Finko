...
**R75 · Un número grande viene con la línea que lo lee:** toda card cuyo protagonista es una cifra lleva, debajo del número, una sola línea derivada de ese mismo dato: qué significa, no qué hacer. Se escribe siempre desde el dato real, nunca como frase fija por estado, y cuando el dato no alcanza para decir algo cierto la línea no se dibuja: la card conserva su vacío. Tipografía de contenido (`sm` sobre `--fk-text-secondary`), no de nota al pie, porque no es un aviso al margen sino la razón por la que la cifra está ahí. Origen: en Análisis, patrimonio, tendencia y categorías mostraban cifras crudas y grandes sin ninguna lectura, mientras el hero del score ya interpretaba la suya con `_fraseScore()`; la asimetría hacía ver el panel como un muro de datos ([ADR 046](DECISIONS/046-analisis-interpreta-criterio-y-lenguaje.md) D3).
