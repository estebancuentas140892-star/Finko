...
- **P4 del ADR 033, resuelto 2026-08-12: el lote completo son las 8 superficies que hoy usan `emptyArt()` geométrico** (`ahorro`, `apartados`, `cuentas`, `deudas`, `inversion`, `metas`, `personales`, `recurring`/Movimientos). No las 6 "más visitadas" que sugería la pregunta abierta del ADR: Inicio, Gastos y Calendario todavía no tienen empty state propio (no llaman `emptyArt()`), así que agregarles uno es una decisión de UX nueva, no una superficie que ya exista para reemplazar. Diseñar esas 3 desde cero es una tarjeta aparte si Esteban la quiere.
- **Catálogo completo (8 piezas, borrador de Claude que Esteban sobrescribe en Illustrator, mismo principio "la biblioteca nace poblada" del ADR 026):** `ahorro.svg`, `apartados.svg`, `cuentas.svg`, `deudas.svg`, `inversion.svg`, `metas.svg`, `personales.svg` y `recurring.svg`, todas con `data-placeholder="true"` (el sync las excluye del sprite hasta que se reemplacen; el empty state sigue usando `emptyArt()` mientras tanto).
